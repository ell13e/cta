...
- 404 handler: `cta-wp-theme/inc/data-importer.php` → `cta_create_missing_static_page_on_404()` (around line 1096).  
- Static page list (includes Group Training): same file, `cta_create_static_pages()` (around line 862).  
- Redirects table: `cta-wp-theme/inc/seo-links-redirects.php` → `cta_process_redirects()` (priority 1 on `template_redirect`).
