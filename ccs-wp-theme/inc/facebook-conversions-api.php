 ...
 */
function ccs_facebook_conversions_api_register_settings() {
    register_setting('ccs_api_keys_settings', 'ccs_facebook_pixel_id', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_facebook_access_token', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_facebook_test_event_code', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_facebook_conversions_api_enabled', [
        'sanitize_callback' => 'absint',
        'default' => 1,
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_facebook_crm_name', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => 'WordPress',
    ]);
}
add_action('admin_init', 'ccs_facebook_conversions_api_register_settings');
 ...
 */
function ccs_send_facebook_event($event_name, $event_data = []) {
    $pixel_id = get_option('ccs_facebook_pixel_id', '');
    $access_token = get_option('ccs_facebook_access_token', '');
    $enabled = get_option('ccs_facebook_conversions_api_enabled', 1);
    $test_event_code = get_option('ccs_facebook_test_event_code', '');
    ...
    if (!empty($event_data['user_data'])) {
        $event['user_data'] = ccs_hash_facebook_user_data($event_data['user_data']);
    } else {
        $event['user_data'] = ccs_get_facebook_user_data();
    }
 ...
 */
function ccs_hash_facebook_user_data($user_data) {
    $hashed = [];
 ...
 */
function ccs_get_facebook_user_data() {
    $user_data = [];
    ...
    return ccs_hash_facebook_user_data($user_data);
}
 ...
 * Called on page load
 */
function ccs_track_facebook_pageview() {
    $pixel_id = get_option('ccs_facebook_pixel_id', '');
    if (empty($pixel_id)) {
        return;
    }
    
    ccs_send_facebook_event('PageView', [
        'event_source_url' => (is_ssl() ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'],
    ]);
}
add_action('wp', 'ccs_track_facebook_pageview');

/**
 * Track Lead event when form submission is saved
 */
function ccs_track_facebook_lead_on_save($post_id, $post, $update) {
    ...
    ccs_track_facebook_lead($post_id, $user_data);
}
add_action('save_post', 'ccs_track_facebook_lead_on_save', 20, 3);

/**
 * Track Purchase event when course booking is completed
 * Hook into course booking controller
 */
function ccs_track_facebook_purchase_on_booking($submission_id, $submission_data) {
    ...
    ccs_track_facebook_purchase($submission_id, $total, 'GBP', $user_data);
}
add_action('ccs_course_booking_saved', 'ccs_track_facebook_purchase_on_booking', 10, 2);
 ...
 */
function ccs_hash_facebook_crm_user_data($user_data) {
    $hashed = [];
 ...
 * @return array|WP_Error Response from Facebook API
 */
function ccs_send_facebook_offline_conversion($lead_id, $event_name, $event_data = [], $crm_name = 'WordPress') {
    $pixel_id = get_option('ccs_facebook_pixel_id', '');
    $access_token = get_option('ccs_facebook_access_token', '');
    $enabled = get_option('ccs_facebook_conversions_api_enabled', 1);
    $test_event_code = get_option('ccs_facebook_test_event_code', '');
    ...
    if (!empty($event_data['user_data'])) {
        $crm_user_data = ccs_hash_facebook_crm_user_data($event_data['user_data']);
 ...
 */
function ccs_track_facebook_offline_conversion_on_status_change($post_id, $old_status, $new_status) {
    ...
    $crm_name = get_option('ccs_facebook_crm_name', 'WordPress');
    
    ccs_send_facebook_offline_conversion($lead_id, $event_name, [
        'custom_data' => $custom_data,
        'event_id' => 'status_' . $post_id . '_' . $new_status . '_' . time(),
    ], $crm_name);
}
add_action('ccs_form_submission_status_changed', 'ccs_track_facebook_offline_conversion_on_status_change', 10, 3);
 ...
 * @return array|WP_Error Response from Facebook API
 */
function ccs_send_manual_offline_conversion($submission_id, $event_name, $event_data = []) {
    ...
    return ccs_send_facebook_offline_conversion($lead_id, $event_name, $event_data, $crm_name);
}
 ...
 */
function ccs_track_facebook_lead($form_id, $form_data) {
    $user_data = [];
 ...
 */
function ccs_track_facebook_purchase($booking_id, $value, $currency = 'GBP', $user_data = []) {
    ccs_send_facebook_event('Purchase', [
        'user_data' => $user_data,
        'custom_data' => [
            'value' => $value,
            'currency' => $currency,
 ...
 */
function ccs_track_facebook_view_content($course_id, $course_title) {
    ccs_send_facebook_event('ViewContent', [
        'custom_data' => [
 ...
 */
function ccs_track_facebook_initiate_checkout($course_id, $value) {
    ccs_send_facebook_event('InitiateCheckout', [
        'custom_data' => [
            'content_ids' => [(string)$course_id],
            'value' => $value,
            'currency' => 'GBP',
 ...
 */
function ccs_facebook_conversions_api_settings_fields() {
    $pixel_id = get_option('ccs_facebook_pixel_id', '');
    $access_token = get_option('ccs_facebook_access_token', '');
    $test_event_code = get_option('ccs_facebook_test_event_code', '');
    $enabled = get_option('ccs_facebook_conversions_api_enabled', 1);
    ...
    <div class="cta-api-keys-section">
        ...
        <table class="form-table">
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_pixel_id">Pixel ID</label>
                </th>
                <td>
                    <input type="text" 
            ...
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_access_token">Access Token</label>
                </th>
                <td>
                    <input type="password" 
            ...
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_test_event_code">Test Event Code</label>
                </th>
                <td>
                    <input type="text" 
            ...
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_conversions_api_enabled">Enable Conversions API</label>
                </th>
                <td>
                    <label>
                        <input type="checkbox" 
            ...
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_crm_name">CRM Name (Conversion Leads)</label>
                </th>
                <td>
                    <input type="text" 
                           id="ccs_facebook_crm_name" 
                           name="ccs_facebook_crm_name" 
