...
function ccs_generate_meta_description($post) {
    ...
    switch ($post_type) {
        case 'course':
            $level = ccs_safe_get_field('course_level', $post->ID, '');
        ...
        case 'course_event':
            ...
            $location = ccs_safe_get_field('event_location', $post->ID, 'Maidstone');
        ...
        case 'post':
            ...
            if ($excerpt) {
                $description = $excerpt . '. Expert guidance from Continuity of Care Services.';
            } else {
...
function ccs_get_meta_description($post = null) {
    ...
    $custom_description = '';
    if ($post_type === 'course') {
        $custom_description = ccs_safe_get_field('course_seo_meta_description', $post->ID, '');
    } elseif ($post_type === 'course_event') {
        $custom_description = ccs_safe_get_field('event_seo_meta_description', $post->ID, '');
    } elseif ($post_type === 'post') {
        $custom_description = ccs_safe_get_field('news_meta_description', $post->ID, '');
    } elseif ($post_type === 'page') {
        // For pages, check page_seo_meta_description first (especially for permanent pages)
        $custom_description = ccs_safe_get_field('page_seo_meta_description', $post->ID, '');
        // Fallback to generic seo_meta_description if page-specific field is empty
        if (empty($custom_description)) {
            $custom_description = ccs_safe_get_field('seo_meta_description', $post->ID, '');
        }
    } else {
        $custom_description = ccs_safe_get_field('seo_meta_description', $post->ID, '');
    }
 ...
 */
function ccs_is_permanent_page($post_id = null) {
    if (!$post_id) {
        global $post;
 ...
 */
function ccs_safe_get_field($field, $post_id = false, $default = '') {
    if ($post_id === 'option') {
        return ccs_get_option_field($field, $default);
    }
 ...
 */
function ccs_safe_update_field($field, $value, $post_id = false) {
 ...
 */
function ccs_get_option_field($field, $default = '') {
    $field_map = [
        'seo_site_description' => 'ccs_seo_site_description',
        'seo_og_image' => null, // Not in Customizer, use default
        'seo_twitter_handle' => 'ccs_seo_twitter_handle',
        'seo_courses_title' => 'ccs_seo_courses_title',
        'seo_courses_description' => 'ccs_seo_courses_description',
        'seo_events_title' => 'ccs_seo_events_title',
        'seo_events_description' => 'ccs_seo_events_description',
        'seo_org_name' => 'ccs_seo_org_name',
        'seo_org_description' => 'ccs_seo_org_description',
        'seo_org_logo' => null,
        'seo_org_founding_date' => 'ccs_seo_founding_year',
        'seo_org_price_range' => null,
        'seo_address_street' => 'ccs_address_line2',
        'seo_address_locality' => 'ccs_address_city',
        'seo_address_region' => null,
        'seo_address_postcode' => 'ccs_address_postcode',
        'seo_geo_lat' => 'ccs_seo_geo_lat',
        'seo_geo_lng' => 'ccs_seo_geo_lng',
        'seo_google_verification' => 'ccs_seo_google_verification',
        'seo_bing_verification' => 'ccs_seo_bing_verification',
        'seo_default_h1_pattern' => 'ccs_seo_default_h1_pattern',
        'seo_default_meta_title_pattern' => 'ccs_seo_default_meta_title_pattern',
        'seo_default_meta_description_template' => 'ccs_seo_default_meta_description_template',
        'seo_default_section_heading' => 'ccs_seo_default_section_heading',
        'contact_phone' => 'ccs_contact_phone',
        'contact_email' => 'ccs_contact_email',
        'social_facebook' => 'ccs_social_facebook',
        'social_instagram' => 'ccs_social_instagram',
        'social_linkedin' => 'ccs_social_linkedin',
        'social_twitter' => 'ccs_social_twitter',
    ];
 ...
 */
function ccs_clean_canonical_url($url) {
...
function ccs_seo_meta_tags() {
    ...
    $site_description = ccs_safe_get_field('seo_site_description', 'option', get_bloginfo('description'));
    
    // Check ACF field first, then WordPress option (for backward compatibility)
    $default_image = ccs_safe_get_field('seo_og_image', 'option', '');
    if (empty($default_image)) {
        $default_image = get_option('ccs_default_og_image', '');
    }
    if (empty($default_image)) {
        $default_image = ccs_image('logo/long_logo-1200w.webp');
    }
    
    // Check ACF field first, then WordPress option (for backward compatibility)
    $twitter_handle = ccs_safe_get_field('seo_twitter_handle', 'option', '');
    if (empty($twitter_handle)) {
        $twitter_handle = get_option('ccs_twitter_handle', '');
    }
    // Remove @ if present (we add it in the meta tag)
    $twitter_handle = ltrim($twitter_handle, '@');
    
    $courses_title = ccs_safe_get_field('seo_courses_title', 'option', 'Professional Training Courses');
    $courses_desc = ccs_safe_get_field('seo_courses_description', 'option', 'Browse our range of CQC-compliant, CPD-accredited care sector training courses in Maidstone, Kent.');
    $events_title = ccs_safe_get_field('seo_events_title', 'option', 'Upcoming Training Courses');
    $events_desc = ccs_safe_get_field('seo_events_description', 'option', 'Book your place on our scheduled training sessions in Maidstone, Kent. View dates and availability.');
    ...
    if (is_singular()) {
        ...
        if (get_post_type() === 'course') {
            $custom_meta_title = ccs_safe_get_field('course_seo_meta_title', get_the_ID(), '');
        } elseif (get_post_type() === 'course_event') {
            $custom_meta_title = ccs_safe_get_field('event_seo_meta_title', get_the_ID(), '');
        } elseif (get_post_type() === 'post') {
            $custom_meta_title = ccs_safe_get_field('news_meta_title', get_the_ID(), '');
        } elseif (get_post_type() === 'page') {
            // For pages, check ACF field first (especially for permanent pages)
            $custom_meta_title = ccs_safe_get_field('page_seo_meta_title', get_the_ID(), '');
        }
        ...
        if (!empty($custom_meta_title)) {
            ...
            $title_pattern = ccs_safe_get_field('seo_default_meta_title_pattern', 'option', '');
        ...
        $custom_canonical = get_post_meta($post->ID, '_ccs_canonical', true);
    ...
    $url = ccs_clean_canonical_url($url);
    $url = esc_url($url);
    $image = esc_url($image);
    ...
    $fb_app_id = get_option('ccs_facebook_app_id', '');
    if (!empty($fb_app_id)) : ?>
    ...
    $google_verify = ccs_safe_get_field('seo_google_verification', 'option', '');
    $bing_verify = ccs_safe_get_field('seo_bing_verification', 'option', '');
 ...
 */
function ccs_output_google_analytics() {
    $ga_id = ccs_get_google_analytics_id();
...
add_action('wp_head', 'ccs_output_google_analytics', 2);
 ...
 */
function ccs_output_google_tag_manager_head() {
    $gtm_id = ccs_get_google_tag_manager_id();
...
add_action('wp_head', 'ccs_output_google_tag_manager_head', 3);
 ...
 */
function ccs_output_google_tag_manager_body() {
    $gtm_id = ccs_get_google_tag_manager_id();
...
add_action('wp_body_open', 'ccs_output_google_tag_manager_body', 1);
 ...
 */
function ccs_output_facebook_pixel() {
    $fb_pixel_id = ccs_get_facebook_pixel_id();
...
add_action('wp_head', 'ccs_output_facebook_pixel', 4);

/**
 * Facebook Pixel noscript fallback (in body so head stays valid for crawlers)
 */
function ccs_output_facebook_pixel_noscript() {
    $fb_pixel_id = ccs_get_facebook_pixel_id();
...
add_action('wp_body_open', 'ccs_output_facebook_pixel_noscript', 2);
...
function ccs_document_title_parts($title_parts) {
    ...
    if (is_singular('course')) {
        ...
        $custom_title = ccs_safe_get_field('course_seo_meta_title', get_the_ID(), '');
...
add_filter('document_title_parts', 'ccs_document_title_parts');

/**
 * Custom title separator (can be overridden by global settings)
 */
function ccs_document_title_separator($sep) {
    $custom_sep = ccs_safe_get_field('seo_title_separator', 'option', '');
    return !empty($custom_sep) ? $custom_sep : '–'; // Default: dash (Rank Math recommendation)
}
add_filter('document_title_separator', 'ccs_document_title_separator');
...
function ccs_schema_markup() {
    $contact = function_exists('ccs_get_contact_info') ? ccs_get_contact_info() : [];
    
    // Get SEO options from admin
    $org_name = ccs_safe_get_field('seo_org_name', 'option', get_bloginfo('name'));
    $org_description = ccs_safe_get_field('seo_org_description', 'option', 'Professional care sector training provider in Maidstone, Kent. CQC-compliant, CPD-accredited courses.');
    $org_logo = ccs_safe_get_field('seo_org_logo', 'option', ccs_image('logo/long_logo-400w.webp'));
    $founding_date = ccs_safe_get_field('seo_org_founding_date', 'option', '2020');
    $price_range = ccs_safe_get_field('seo_org_price_range', 'option', '££');
    
    // Address from SEO options
    $street = ccs_safe_get_field('seo_address_street', 'option', 'The Maidstone Studios, New Cut Road');
    $locality = ccs_safe_get_field('seo_address_locality', 'option', 'Maidstone');
    $region = ccs_safe_get_field('seo_address_region', 'option', 'Kent');
    $postcode = ccs_safe_get_field('seo_address_postcode', 'option', 'ME14 5NZ');
    $lat = ccs_safe_get_field('seo_geo_lat', 'option', '51.2795');
    $lng = ccs_safe_get_field('seo_geo_lng', 'option', '0.5467');
    
    // Get Trustpilot rating for aggregate rating schema
    $trustpilot_rating = ccs_get_theme_option('trustpilot_rating', '4.6/5');
    $trustpilot_url = ccs_get_theme_option('trustpilot_url', 'https://uk.trustpilot.com/review/continuitytrainingacademy.co.uk');
    $review_count = absint(ccs_get_theme_option('trustpilot_review_count', 20));
    ...
    // Course schema: output by seo-schema.php (ccs_output_course_schema) on single course pages.
    
    // Course Event schema
    if (is_singular('course_event')) {
        global $post;
        $linked_course = ccs_safe_get_field('linked_course', $post->ID, null);
        $event_date = ccs_safe_get_field('event_date', $post->ID, '');
        $start_time = ccs_safe_get_field('start_time', $post->ID, '');
        $location = ccs_safe_get_field('event_location', $post->ID, '');
        $spaces = ccs_safe_get_field('spaces_available', $post->ID, '');
        $price = ccs_safe_get_field('event_price', $post->ID, '');
        
        if (!$price && $linked_course) {
            $price = ccs_safe_get_field('course_price', $linked_course->ID, '');
        }
    ...
    if (is_singular('post')) {
        ...
        $article_schema = [
            ...
            'publisher' => [
                ...
                'logo' => [
                    '@type' => 'ImageObject',
                    'url' => ccs_image('logo/long_logo-400w.webp'),
                ],
            ],
            'mainEntityOfPage' => [
                '@type' => 'WebPage',
    ...
    if (is_page()) {
        ...
        if (empty($faqs) && function_exists('ccs_get_page_faqs')) {
            $faqs = ccs_get_page_faqs($post->ID);
        }
    ...
    if (is_page() && !ccs_is_permanent_page()) {
        global $post;
        $webpage_schema = [
            '@context' => 'https://schema.org',
            '@type' => 'WebPage',
            ...
            'url' => get_permalink(),
            'name' => get_the_title(),
            'description' => ccs_get_meta_description(),
            'isPartOf' => [
                '@id' => home_url('/#website'),
            ],
            'about' => [
    ...
    if (!is_front_page() && !is_singular('course')) {
        $breadcrumbs = ccs_get_breadcrumb_items();
...
add_action('wp_head', 'ccs_schema_markup', 5);
 ...
 */
function ccs_prevent_sitemap_output() {
    ...
    if (strpos($request_uri, '/wp-sitemap') !== false) {
        ...
        remove_action('wp_head', 'ccs_seo_meta_tags', 1);
        remove_action('wp_head', 'ccs_output_google_analytics', 2);
        remove_action('wp_head', 'ccs_output_google_tag_manager_head', 3);
        remove_action('wp_head', 'ccs_output_facebook_pixel', 4);
    }
}
add_action('init', 'ccs_prevent_sitemap_output', 1);

/**
 * Add custom post types to WordPress sitemap
 */
function ccs_sitemap_post_types($post_types) {
...
add_filter('wp_sitemaps_post_types', 'ccs_sitemap_post_types');

/**
 * Remove taxonomies from sitemap
 * Only include individual posts/pages, not taxonomy archives
 */
function ccs_sitemap_taxonomies($taxonomies) {
...
add_filter('wp_sitemaps_taxonomies', 'ccs_sitemap_taxonomies');

/**
 * Ensure taxonomy sitemap entries have a valid loc; fix from term link if missing.
 */
function ccs_filter_sitemap_taxonomies_entry($entry, $term_id, $taxonomy, $term) {
...
add_filter('wp_sitemaps_taxonomies_entry', 'ccs_filter_sitemap_taxonomies_entry', 10, 4);

/**
 * Exclude author archives from sitemap (noindexed per SEO docs)
 */
function ccs_sitemap_exclude_users($args, $user) {
    // Exclude all users from sitemap since author archives are noindexed
    return false;
}
add_filter('wp_sitemaps_users_query_args', 'ccs_sitemap_exclude_users', 10, 2);

/**
 * Remove users provider from sitemap entirely
 */
function ccs_sitemap_remove_users($provider, $name) {
    if ($name === 'users') {
        return false;
    }
    return $provider;
}
add_filter('wp_sitemaps_add_provider', 'ccs_sitemap_remove_users', 10, 2);

/**
 * Filter sitemap entries to exclude noindexed content
 * This runs after the query, so we can check each post individually
 */
function ccs_filter_sitemap_entry($entry, $post, $post_type) {
    ...
    $noindex = get_post_meta($post->ID, '_ccs_noindex', true);
...
add_filter('wp_sitemaps_posts_entry', 'ccs_filter_sitemap_entry', 5, 3);

/**
 * Set priority and change frequency for sitemap entries
 */
function ccs_sitemap_entry($entry, $post, $post_type) {
...
add_filter('wp_sitemaps_posts_entry', 'ccs_sitemap_entry', 10, 3);

/**
 * Check if a post's post_modified_gmt is valid for sitemap lastmod (same rules as ccs_sitemap_entry).
 */
function ccs_sitemap_lastmod_is_valid($post) {
    if (empty($post->post_modified_gmt) || $post->post_modified_gmt === '0000-00-00 00:00:00') {
        return false;
    }
    $ts = strtotime($post->post_modified_gmt);
 ...
 */
function ccs_fix_sitemap_lastmod_dates() {
    ...
    foreach ($post_types as $post_type) {
        ...
        foreach ($posts as $post_id) {
            $post = get_post($post_id);
            if (!$post || ccs_sitemap_lastmod_is_valid($post)) {
                continue;
            }
 ...
 */
function ccs_sitemap_max_urls($max_urls) {
    return 2000; // Increased from default
}
add_filter('wp_sitemaps_max_urls', 'ccs_sitemap_max_urls');

/**
 * Flush sitemap cache when posts are added/updated
 */
function ccs_flush_sitemap_cache($post_id, $post) {
    ...
    if (in_array($post->post_type, $sitemap_post_types) && $post->post_status === 'publish') {
        ...
        ccs_ping_search_engines();
    }
}
add_action('save_post', 'ccs_flush_sitemap_cache', 10, 2);
add_action('delete_post', 'ccs_flush_sitemap_cache', 10, 2);

/**
 * Flush sitemap cache when terms are added/updated
 */
function ccs_flush_sitemap_taxonomy_cache($term_id, $tt_id, $taxonomy) {
...
add_action('create_term', 'ccs_flush_sitemap_taxonomy_cache', 10, 3);
add_action('edit_term', 'ccs_flush_sitemap_taxonomy_cache', 10, 3);
add_action('delete_term', 'ccs_flush_sitemap_taxonomy_cache', 10, 3);

/**
 * Ping search engines when sitemap updates
 */
function ccs_ping_search_engines() {
    // Only ping once per hour max
    $last_ping = get_transient('ccs_sitemap_last_ping');
    if ($last_ping) {
        return;
    }
    ...
    set_transient('ccs_sitemap_last_ping', time(), HOUR_IN_SECONDS);
}

/**
 * Daily cleanup - flush sitemap cache to remove past events
 */
function ccs_cleanup_sitemap() {
...
add_action('ccs_daily_cleanup', 'ccs_cleanup_sitemap');
 ...
 */
function ccs_schedule_daily_cleanup() {
    if (!wp_next_scheduled('ccs_daily_cleanup')) {
        ...
        wp_schedule_event($next_3am_gmt, 'daily', 'ccs_daily_cleanup');
    }
}
add_action('after_switch_theme', 'ccs_schedule_daily_cleanup');
 ...
 */
function ccs_fix_duplicate_urls_on_activation() {
    $fixes = [
...
add_action('after_switch_theme', 'ccs_fix_duplicate_urls_on_activation', 5);
...
add_action('init', function() {
    ...
    $fix_run = get_option('ccs_duplicate_urls_fixed', false);
    if ($fix_run) {
        return; // Already fixed
    }
    
    // Run the fix
    ccs_fix_duplicate_urls_on_activation();
    
    // Mark as done
    update_option('ccs_duplicate_urls_fixed', true);
}, 1);
 ...
 */
function ccs_add_sitemap_admin_menu() {
...
// add_action('admin_menu', 'ccs_add_sitemap_admin_menu', 20);

/**
 * Sitemap admin page
 */
function ccs_sitemap_admin_page() {
    // Handle manual sitemap refresh
    if (isset($_POST['refresh_sitemap']) && check_admin_referer('ccs_refresh_sitemap')) {
        // Clear all sitemap caches
        wp_cache_flush();
        delete_transient('wp_sitemap_index');
        delete_transient('ccs_sitemap_last_ping');
    ...
    if (isset($_POST['fix_lastmod_dates']) && check_admin_referer('ccs_fix_lastmod_dates')) {
        $count = ccs_fix_sitemap_lastmod_dates();
        wp_cache_flush();
        delete_transient('wp_sitemap_index');
    ...
    <div class="wrap cta-seo-page">
        ...
        <div class="cta-seo-section">
            ...
            <form method="post" style="margin-bottom: 1em;">
                <?php wp_nonce_field('ccs_refresh_sitemap'); ?>
            ...
            <form method="post">
                <?php wp_nonce_field('ccs_fix_lastmod_dates'); ?>
 ...
 */
function ccs_sitemap_diagnostic_notice() {
...
add_action('admin_notices', 'ccs_sitemap_diagnostic_notice');

/**
 * Handle one-click fixes via AJAX
 */
function ccs_sitemap_fix_handler() {
    // Verify nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'ccs_sitemap_fix')) {
...
add_action('wp_ajax_ccs_sitemap_fix', 'ccs_sitemap_fix_handler');
 ...
 */
function ccs_add_sitemap_diagnostic_page() {
    // Menu registration moved to seo-admin.php
    // This function is kept for backward compatibility
}
// Removed: add_action('admin_menu', 'ccs_add_sitemap_diagnostic_page');

/**
 * Sitemap diagnostic page content
 */
function ccs_sitemap_diagnostic_page() {
    $sitemap_url = home_url('/wp-sitemap.xml');
 ...
 */
function ccs_robots_txt($output, $public) {
    if (!$public) {
        return $output;
    }

    $output = "# robots.txt for Continuity of Care Services\n\n";
...
add_filter('robots_txt', 'ccs_robots_txt', 10, 2);
 ...
 */
function ccs_get_orphan_link_suggestions($orphan_id) {
    ...
    $orphan_keywords = ccs_extract_keywords_from_content($orphan_title . ' ' . wp_strip_all_tags($orphan_content));
    ...
    foreach ($source_posts as $source) {
        ...
        $relevance = ccs_calculate_content_relevance(
            $orphan_keywords,
        ...
        if ($relevance >= 30) {
            $suggestions[] = [
                ...
                'suggested_keyword' => ccs_find_best_keyword_match($orphan_keywords, $source_content)
            ];
        }
    }
 ...
 */
function ccs_extract_keywords_from_content($content) {
 ...
 */
function ccs_calculate_content_relevance($orphan_keywords, $source_content, $orphan_type, $source_type) {
 ...
 */
function ccs_find_best_keyword_match($keywords, $content) {
 ...
 */
function ccs_seo_tools_admin_page() {
    // Handle CSV import
    $import_result = null;
    if (isset($_POST['import_csv']) && check_admin_referer('ccs_import_seo_csv')) {
        $import_result = ccs_import_meta_descriptions_from_csv();
    }
    ...
    foreach ($post_types as $post_type) {
        ...
        foreach ($posts as $post_id) {
            $has_meta = false;
            if ($post_type === 'course') {
                $has_meta = !empty(ccs_safe_get_field('course_seo_meta_description', $post_id, ''));
            } elseif ($post_type === 'course_event') {
                $has_meta = !empty(ccs_safe_get_field('event_seo_meta_description', $post_id, ''));
            } elseif ($post_type === 'post') {
                $has_meta = !empty(ccs_safe_get_field('news_meta_description', $post_id, ''));
            } else {
                $has_meta = !empty(ccs_safe_get_field('seo_meta_description', $post_id, ''));
            }
    ...
    <div class="wrap">
                ...
                <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd;">
                    ...
                    <?php if ($csv_exists) : ?>
                        ...
                        <form method="post" style="margin-top: 15px;">
                            <?php wp_nonce_field('ccs_import_seo_csv'); ?>
                ...
                <div class="notice notice-warning inline" style="margin-top: 15px;">
                    ...
                    <div style="margin-top: 20px; padding: 16px; background: #f0f6fc; border-left: 4px solid #2271b1; border-radius: 4px;">
                        ...
                        <form method="post" action="<?php echo esc_url(admin_url('admin.php?page=cta-seo-tools&tab=orphan-fixer')); ?>" style="margin-top: 15px;">
                            <?php wp_nonce_field('ccs_fix_orphan_pages'); ?>
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'fix_orphan_pages' && check_admin_referer('ccs_fix_orphan_pages')) {
            $orphan_id = isset($_POST['orphan_id']) ? absint($_POST['orphan_id']) : 0;
            if ($orphan_id > 0) {
                $suggestions = ccs_get_orphan_link_suggestions($orphan_id);
                $show_suggestions = true;
            } else {
                // Show all orphan pages with suggestions
                $all_orphans_with_suggestions = [];
                foreach ($orphan_pages as $orphan) {
                    $suggestions = ccs_get_orphan_link_suggestions($orphan['id']);
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'apply_orphan_links' && check_admin_referer('ccs_apply_orphan_links')) {
        ...
        <div class="card" style="margin-top: 20px;">
            ...
            <?php if (isset($all_orphans_with_suggestions)) : ?>
                <form method="post" id="orphan-links-form" action="<?php echo esc_url(admin_url('admin.php?page=cta-seo-tools&tab=orphan-fixer')); ?>">
                    <?php wp_nonce_field('ccs_apply_orphan_links'); ?>
        ...
        <div class="card" style="margin-top: 20px;">
            ...
            $fix_result = null;
            if (isset($_POST['fix_duplicate_urls']) && check_admin_referer('ccs_fix_duplicate_urls')) {
                $fix_result = ccs_fix_duplicate_course_urls();
            }
            ...
            <?php if (array_filter($status, function($s) { return $s['status'] === 'needs_fix'; })) : ?>
                <form method="post" style="margin-top: 20px;">
                    <?php wp_nonce_field('ccs_fix_duplicate_urls'); ?>
 ...
 */
function ccs_fix_duplicate_course_urls() {
    $fixes = [
        'adult-social-care-certificate' => 'adult-social-care-certificate-3d-l2',
        'emergency-first-aid-at-work' => 'emergency-first-aid-at-work-1d-l3',
        'medication-competency-management' => 'medication-competency-management-1d-l3',
 ...
 */
function ccs_resource_hints($urls, $relation_type) {
...
add_filter('wp_resource_hints', 'ccs_resource_hints', 10, 2);

/**
 * Remove unnecessary meta tags and prevent duplicate canonicals
 */
function ccs_cleanup_head() {
    ...
    // We output our own canonical in ccs_seo_meta_tags()
...
add_action('after_setup_theme', 'ccs_cleanup_head');

/**
 * Add robots meta for specific pages
 */
function ccs_robots_meta($robots) {
    // Check per-page settings first (from SEO Controls meta box)
    if (is_singular()) {
        $post_id = get_queried_object_id();
        $noindex = get_post_meta($post_id, '_ccs_noindex', true);
        $nofollow = get_post_meta($post_id, '_ccs_nofollow', true);
        $nosnippet = get_post_meta($post_id, '_ccs_nosnippet', true);
        $max_snippet = get_post_meta($post_id, '_ccs_max_snippet_length', true);
...
add_filter('wp_robots', 'ccs_robots_meta');
 ...
 */
function ccs_enforce_canonical_redirect() {
...
add_action('template_redirect', 'ccs_enforce_canonical_redirect', 5);

/**
 * Handle specific URL redirects (404 fixes, old URLs, etc.)
 */
function ccs_handle_specific_redirects() {
    // Don't redirect in admin, AJAX, or for logged-in users editing
    if (is_admin() || wp_doing_ajax() || is_preview()) {
        return;
    }
...
add_action('template_redirect', 'ccs_handle_specific_redirects', 1);
 ...
 */
function ccs_auto_alt_text($attr, $attachment, $size) {
    if (empty($attr['alt'])) {
        ...
        if ($parent_id && get_post_type($parent_id) === 'course') {
            ...
            $linked_course = ccs_safe_get_field('linked_course', $parent_id, null);
            if ($linked_course) {
...
add_filter('wp_get_attachment_image_attributes', 'ccs_auto_alt_text', 10, 3);

/**
 * Add loading="lazy" and decoding="async" to images
 */
function ccs_lazy_load_images($attr) {
...
add_filter('wp_get_attachment_image_attributes', 'ccs_lazy_load_images');
 ...
 */
function ccs_auto_link_courses($content) {
    if (!is_singular('post')) {
        return $content;
    }
...
add_filter('the_content', 'ccs_auto_link_courses', 20);
 ...
 */
function ccs_add_seo_dashboard_widget() {
    wp_add_dashboard_widget(
        'ccs_seo_widget',
        'SEO Status & Tools',
        'ccs_seo_dashboard_widget_content'
    );
}
add_action('wp_dashboard_setup', 'ccs_add_seo_dashboard_widget');

/**
 * SEO dashboard widget content
 */
function ccs_seo_dashboard_widget_content() {
    // Get verification results (cached)
    $verification_results = get_transient('ccs_seo_verification_results');
    
    if ($verification_results === false && function_exists('ccs_run_seo_verification')) {
        $verification_results = ccs_run_seo_verification();
        set_transient('ccs_seo_verification_results', $verification_results, HOUR_IN_SECONDS);
    }
    ...
    <div>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <ul style="margin: 10px 0; padding-left: 20px; font-size: 12px;">
                <li>
                    ...
                    <?php if (function_exists('ccs_schema_markup')) : ?>
                ...
                <li>
                    ...
                    <?php 
                    $rating = ccs_get_theme_option('trustpilot_rating', '');
                    if (!empty($rating)) :
                    ?>
        ...
        <div style="margin-bottom: 15px; padding-top: 15px; border-top: 1px solid #e0e0e0;">
            ...
            <ul style="margin: 10px 0; padding-left: 20px; font-size: 12px;">
                ...
                <?php endif; ?>
                <?php if (empty(ccs_get_theme_option('trustpilot_rating', ''))) : ?>
                <li style="color: #dba617;">Configure Trustpilot rating</li>
                <?php endif; ?>
                <?php if (get_option('blog_public') == 1 && !empty(get_option('permalink_structure')) && !empty(ccs_get_theme_option('trustpilot_rating', ''))) : ?>
 ...
 */
function ccs_check_empty_anchors($content) {
...
add_filter('content_save_pre', 'ccs_check_empty_anchors');
 ...
 */
function ccs_auto_generate_seo_meta($post_id, $post, $update) {
    ...
    if ($post->post_type === 'course') {
        $existing = ccs_safe_get_field('course_seo_meta_description', $post_id, '');
    } elseif ($post->post_type === 'course_event') {
        $existing = ccs_safe_get_field('event_seo_meta_description', $post_id, '');
    } elseif ($post->post_type === 'post') {
        $existing = ccs_safe_get_field('news_meta_description', $post_id, '');
    } else {
        $existing = ccs_safe_get_field('seo_meta_description', $post_id, '');
    }
    ...
    $description = ccs_generate_meta_description($post);
    
    if (!empty($description)) {
        // Save to appropriate field using ACF-aware update function
        if ($post->post_type === 'course') {
            ccs_safe_update_field('course_seo_meta_description', $description, $post_id);
        } elseif ($post->post_type === 'course_event') {
            ccs_safe_update_field('event_seo_meta_description', $description, $post_id);
        } elseif ($post->post_type === 'post') {
            ccs_safe_update_field('news_meta_description', $description, $post_id);
        } else {
            ccs_safe_update_field('seo_meta_description', $description, $post_id);
        }
    }
}
add_action('save_post', 'ccs_auto_generate_seo_meta', 10, 3);
 ...
 */
function ccs_import_meta_descriptions_from_csv() {
    $csv_path = get_template_directory() . '/data/seo_meta_descriptions.csv';
