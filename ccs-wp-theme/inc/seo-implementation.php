 ...
 */
function ccs_register_event_seo_fields() {
...
add_action('acf/init', 'ccs_register_event_seo_fields');
 ...
 */
function ccs_hook_output_event_preload_hints() {
    ...
    $preload_hints = ccs_get_hero_image_preload_hints($featured_image_id, true);

    // Output preload links
    ccs_output_resource_hints($preload_hints);
}
add_action('wp_head', 'ccs_hook_output_event_preload_hints', 5);
 ...
 */
function ccs_get_related_events($post_id = null, $limit = 3) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
    ...
    // Get linked course
    $linked_course = function_exists('ccs_safe_get_field') ? ccs_safe_get_field('linked_course', $post_id) : null;
    if (!$linked_course) {
        return [];
    }
    ...
    $cache_key = 'ccs_related_events_' . $post_id . '_' . $limit;
 ...
 * @return void
 */
function ccs_display_related_events($post_id = null, $limit = 3) {
    // Validate $post_id
    if ($post_id && !is_numeric($post_id)) {
        return;
    }

    $related_events = ccs_get_related_events($post_id, $limit);
            ...
            <div class="events-grid">
                <?php foreach ($related_events as $event) : ?>
                    <article class="event-card">
                        ...
                        <?php 
                        $event_date = function_exists('ccs_safe_get_field') ? ccs_safe_get_field('event_date', $event->ID) : get_post_meta($event->ID, 'event_date', true);
                        if ($event_date) : 
                        ?>
 ...
 */
function ccs_add_event_seo_checklist_metabox() {
    add_meta_box(
        'ccs_event_seo_checklist',
        '✓ SEO Checklist for Event Pages',
        'ccs_event_seo_checklist_callback',
        'course_event',
        'side',
        'high'
    );
}
add_action('add_meta_boxes', 'ccs_add_event_seo_checklist_metabox');

/**
 * Render SEO checklist
 */
function ccs_event_seo_checklist_callback($post) {
    // Helper function to safely get field value
    $get_field_value = function($field_name, $post_id) {
        if (function_exists('ccs_safe_get_field')) {
            return ccs_safe_get_field($field_name, $post_id);
 ...
 */
function ccs_add_schema_validation_admin_notice() {
...
add_action('admin_notices', 'ccs_add_schema_validation_admin_notice');
 ...
 */
function ccs_log_seo_implementation() {
    if (!is_admin()) {
        return;
    }
    ...
    $implementation_log = [
        ...
        'event_schema' => function_exists('ccs_get_event_schema'),
        'cwv_optimization' => function_exists('ccs_get_responsive_image_html'),
        'breadcrumb_schema' => function_exists('ccs_get_breadcrumb_schema'),
        'related_events' => function_exists('ccs_get_related_events'),
    ];

    // Log as transient (expires in 24 hours)
    set_transient('ccs_seo_implementation_status', $implementation_log, 24 * HOUR_IN_SECONDS);
}
add_action('init', 'ccs_log_seo_implementation');
 ...
 * <?php echo ccs_the_event_featured_image(['lazy' => true]); ?>
 * 
 * @param array $args Image arguments
 * @return string HTML image markup
 */
function ccs_the_event_featured_image($args = []) {
    $post_id = get_the_ID();
    if (!$post_id) {
        return '';
    }
    ...
    $args = apply_filters('ccs_featured_image_args', $args, $post_id);

    // Ensure cwv-optimization module is loaded
    if (!function_exists('ccs_get_responsive_image_html')) {
        return '';
    }

    return ccs_get_responsive_image_html($featured_image_id, $args['size'], $args);
}
 ...
 */
apply_filters('ccs_use_course_instance_schema', false);
apply_filters('ccs_use_event_schema', true);
