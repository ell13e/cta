 ...
 */
function ccs_get_opening_hours_schema() {
    $weekdays = get_theme_mod('ccs_opening_weekdays', '');
    $saturday = get_theme_mod('ccs_opening_saturday', '');
    $sunday = get_theme_mod('ccs_opening_sunday', '');
    $specs = [];
    $day_ranges = [
...
function ccs_get_organization_schema() {
    ...
    $show_rating = get_theme_mod('ccs_show_trustpilot_rating_schema', true);
    $rating_value = get_theme_mod('ccs_trustpilot_rating', '4.6');
    $review_count = get_theme_mod('ccs_trustpilot_review_count', '20');
    
    // Get social media URLs from theme customizer
    $facebook_url = get_theme_mod('ccs_facebook_url', 'https://www.facebook.com/continuitytraining');
    $linkedin_url = get_theme_mod('ccs_linkedin_url', 'https://www.linkedin.com/company/continuity-training-academy-cta');
    $instagram_url = get_theme_mod('ccs_instagram_url', 'https://www.instagram.com/continuitytrainingacademy');
    $trustpilot_url = get_theme_mod('ccs_trustpilot_url', 'https://www.trustpilot.com/review/continuitytrainingacademy.co.uk');
    ...
    $opening = ccs_get_opening_hours_schema();
    if (!empty($opening)) {
 ...
 */
function ccs_get_course_schema($post_id = null) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
    ...
    $duration = ccs_safe_get_field('course_duration', $post_id, '');
    ...
    $level = ccs_safe_get_field('course_level', $post_id, '');
    if ($level) {
        $course['educationalLevel'] = $level;
    }
    $accreditation = ccs_safe_get_field('course_accreditation', $post_id, '');
    if ($accreditation) {
        $course['educationalCredentialAwarded'] = $accreditation;
    }
    $price = ccs_safe_get_field('course_price', $post_id, '');
    if ($price !== '' && $price !== null) {
 ...
 */
function ccs_output_course_schema() {
    ...
    $graph = array_merge(
        ccs_get_course_schema(),
        [ccs_get_organization_schema()],
        [ccs_get_breadcrumb_schema($breadcrumb_items)]
    );
    ccs_output_schema_json($graph);
}
add_action('wp_head', 'ccs_output_course_schema', 10);

/**
 * Get page featured image URL for schema
 * Checks for custom schema image first, then falls back to featured image
 */
function ccs_get_page_schema_image($post_id = null) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
    
    // Check for custom schema image (meta field)
    $schema_image_id = get_post_meta($post_id, '_ccs_schema_image', true);
 ...
 */
function ccs_get_breadcrumb_schema($items = []) {
    $site_url = home_url();
    $page_url = get_permalink();
 ...
 * Get WebPage schema
 */
function ccs_get_webpage_schema($args = []) {
    $site_url = home_url();
    $page_url = get_permalink();
    
    $defaults = [
        'name' => get_the_title() . ' | Continuity of Care Services',
    ...
    $image_url = ccs_get_page_schema_image();
    if ($image_url) {
 ...
 */
function ccs_output_schema_json($schema_graph) {
 ...
 */
function ccs_add_schema_image_meta_box() {
    add_meta_box(
        'ccs_schema_image',
        'Schema.org Featured Image',
        'ccs_schema_image_meta_box_callback',
        ['page', 'post', 'course'],
        'side',
        'low'
    );
}
add_action('add_meta_boxes', 'ccs_add_schema_image_meta_box');

/**
 * Schema image meta box callback
 */
function ccs_schema_image_meta_box_callback($post) {
    wp_nonce_field('ccs_schema_image_nonce', 'ccs_schema_image_nonce');
    
    $image_id = get_post_meta($post->ID, '_ccs_schema_image', true);
    ...
    <div class="cta-schema-image-wrapper">
        ...
        <input type="hidden" id="ccs_schema_image_id" name="ccs_schema_image_id" value="<?php echo esc_attr($image_id); ?>" />
    ...
    jQuery(document).ready(function($) {
        ...
        $('.cta-upload-schema-image').on('click', function(e) {
            ...
            mediaUploader.on('select', function() {
                ...
                $('#ccs_schema_image_id').val(attachment.id);
 ...
 */
function ccs_save_schema_image_meta($post_id) {
    // Check nonce
    if (!isset($_POST['ccs_schema_image_nonce']) || !wp_verify_nonce($_POST['ccs_schema_image_nonce'], 'ccs_schema_image_nonce')) {
        return;
    }
    ...
    // Save or delete meta
    if (isset($_POST['ccs_schema_image_id']) && !empty($_POST['ccs_schema_image_id'])) {
        update_post_meta($post_id, '_ccs_schema_image', intval($_POST['ccs_schema_image_id']));
    } else {
        delete_post_meta($post_id, '_ccs_schema_image');
    }
}
add_action('save_post', 'ccs_save_schema_image_meta');

/**
 * Add theme customizer settings for schema
 */
function ccs_schema_customizer_settings($wp_customize) {
    // Add Schema Section
    $wp_customize->add_section('ccs_schema_settings', [
    ...
    // Social Media URLs
    $wp_customize->add_setting('ccs_facebook_url', [
        'default' => 'https://www.facebook.com/continuitytraining',
        'sanitize_callback' => 'esc_url_raw',
    ]);
    
    $wp_customize->add_control('ccs_facebook_url', [
        'label' => 'Facebook URL',
        'section' => 'ccs_schema_settings',
        'type' => 'url',
    ]);
    
    $wp_customize->add_setting('ccs_linkedin_url', [
        'default' => 'https://www.linkedin.com/company/continuity-training-academy-cta',
        'sanitize_callback' => 'esc_url_raw',
    ]);
    
    $wp_customize->add_control('ccs_linkedin_url', [
        'label' => 'LinkedIn URL',
        'section' => 'ccs_schema_settings',
        'type' => 'url',
    ]);
    
    $wp_customize->add_setting('ccs_instagram_url', [
        'default' => 'https://www.instagram.com/continuitytrainingacademy',
        'sanitize_callback' => 'esc_url_raw',
    ]);
    
    $wp_customize->add_control('ccs_instagram_url', [
        'label' => 'Instagram URL',
        'section' => 'ccs_schema_settings',
        'type' => 'url',
    ]);
    ...
    $wp_customize->add_setting('ccs_show_trustpilot_rating_schema', [
        'default' => true,
        'sanitize_callback' => function ($v) { return (bool) $v; },
    ]);
    $wp_customize->add_control('ccs_show_trustpilot_rating_schema', [
    ...
    $wp_customize->add_setting('ccs_opening_weekdays', [
        'default' => '',
        'sanitize_callback' => 'sanitize_text_field',
    ]);
    $wp_customize->add_control('ccs_opening_weekdays', [
        'label' => 'Opening hours (Mon–Fri)',
        'description' => 'e.g. 09:00-17:00. Leave blank to omit.',
        'section' => 'ccs_schema_settings',
        'type' => 'text',
    ]);
    $wp_customize->add_setting('ccs_opening_saturday', [
        'default' => '',
        'sanitize_callback' => 'sanitize_text_field',
    ]);
    $wp_customize->add_control('ccs_opening_saturday', [
        'label' => 'Opening hours (Saturday)',
        'description' => 'e.g. 09:00-12:00',
        'section' => 'ccs_schema_settings',
        'type' => 'text',
    ]);
    $wp_customize->add_setting('ccs_opening_sunday', [
        'default' => '',
        'sanitize_callback' => 'sanitize_text_field',
    ]);
    $wp_customize->add_control('ccs_opening_sunday', [
...
add_action('customize_register', 'ccs_schema_customizer_settings');
 ...
 */
function ccs_get_homepage_schema() {
    $site_url = home_url();
    ...
    $schema_graph[] = ccs_get_local_business_schema();
    
    // BreadcrumbList for homepage
    $schema_graph[] = ccs_get_breadcrumb_schema([
        ['name' => 'Home', 'url' => $site_url . '/'],
    ]);
 ...
 */
function ccs_get_about_page_schema() {
    $site_url = home_url();
    $page_url = get_permalink();
    $page_title = get_the_title();
    $page_description = ccs_get_meta_description();
 ...
 */
function ccs_get_contact_page_schema() {
    $site_url = home_url();
    $page_url = get_permalink();
    $page_title = get_the_title();
    $page_description = ccs_get_meta_description();
 ...
 */
function ccs_get_collection_page_schema($page_slug = '') {
    $site_url = home_url();
    $page_url = get_permalink();
    $page_title = get_the_title();
    $page_description = ccs_get_meta_description();
 ...
 * Get FAQPage schema
 */
function ccs_get_faq_page_schema() {
    $site_url = home_url();
    $page_url = get_permalink();
    $page_title = get_the_title();
    $page_description = ccs_get_meta_description();
 ...
 */
function ccs_get_group_training_page_schema() {
    $site_url = home_url();
    $page_url = get_permalink();
    $page_title = get_the_title();
    $page_description = ccs_get_meta_description();
 ...
 */
function ccs_output_permanent_page_schema() {
    // Only on pages
    if (!is_page()) {
        return;
    }
    ...
    if (!function_exists('ccs_is_permanent_page')) {
        return;
    }
    
    if (!ccs_is_permanent_page()) {
        return;
    }
    
    $page_slug = get_post()->post_name;
    $schema_type = ccs_safe_get_field('page_schema_type', get_the_ID(), '');
    
    $schema_graph = [];
    
    // Determine schema type based on page slug or ACF field
    if (is_front_page() || $page_slug === 'home') {
        $schema_graph = ccs_get_homepage_schema();
    } elseif ($page_slug === 'about' || $page_slug === 'about-us') {
        $schema_graph = ccs_get_about_page_schema();
    } elseif ($page_slug === 'contact') {
        $schema_graph = ccs_get_contact_page_schema();
    } elseif ($page_slug === 'group-training') {
        $schema_graph = ccs_get_group_training_page_schema();
    } elseif ($page_slug === 'faqs') {
        $schema_graph = ccs_get_faq_page_schema();
    } elseif (in_array($page_slug, ['cqc-compliance-hub', 'downloadable-resources', 'news'], true)) {
        $schema_graph = ccs_get_collection_page_schema($page_slug);
    } else {
        // Fallback to WebPage schema
        $schema_graph = [
            ccs_get_webpage_schema([
                'name' => get_the_title(),
                'description' => ccs_get_meta_description(),
            ]),
            ccs_get_organization_schema(),
            ccs_get_breadcrumb_schema(),
        ];
    }
...
add_action('wp_head', 'ccs_output_permanent_page_schema', 10);
