 ...
 */
function ccs_ajax_generate_ai_alt_text() {
    check_ajax_referer('ccs_generate_ai_alt_text', 'nonce');
    ...
    $context = ccs_ai_alt_text_context($attachment_id);
    $result = ccs_ai_alt_text_call_vision($image_url, $context, $attachment_id);
...
add_action('wp_ajax_ccs_generate_ai_alt_text', 'ccs_ajax_generate_ai_alt_text');

/**
 * Gather context for alt text (parent post, site name, filename)
 */
function ccs_ai_alt_text_context($attachment_id) {
    $context = [
 ...
 */
function ccs_ai_alt_text_call_vision($image_url, array $context, $attachment_id = 0) {
    $system = ccs_ai_alt_text_system_prompt($context);
    $user_text = ccs_ai_alt_text_user_prompt();

    $providers = ccs_ai_get_attemptable_providers('openai');
    ...
    foreach ($vision_providers as $provider) {
        ...
        if ($provider === 'openai') {
            $out = ccs_ai_alt_text_openai_vision($key, $system, $user_text, $image_url);
        } else {
            if (empty($image_b64)) {
                continue;
            }
            $out = ccs_ai_alt_text_anthropic_vision($key, $system, $user_text, $image_b64, $mime_type);
        }
        if (is_wp_error($out)) {
            continue;
        }
        $parsed = ccs_ai_alt_text_parse_response($out);
 ...
 */
function ccs_ai_alt_text_parse_response($raw) {
    $raw = trim((string) $raw);
    if ($raw === '') {
        return [];
    }
 ...
 */
function ccs_ai_alt_text_user_prompt() {
...
function ccs_ai_alt_text_system_prompt(array $context) {
    $parts = [
        'You generate WordPress media metadata (title, caption, alt text, description) for a UK care sector training website.',
        'Site: ' . (isset($context['site_name']) ? $context['site_name'] : 'Continuity of Care Services') . '.',
 ...
 */
function ccs_ai_alt_text_openai_vision($api_key, $system, $user_text, $image_url) {
    $body = [
        'model'       => 'gpt-4o',
        'max_tokens'  => 400,
        'temperature' => 0.4,
 ...
 */
function ccs_ai_alt_text_anthropic_vision($api_key, $system, $user_text, $image_data, $mime_type = 'image/jpeg') {
 ...
 */
function ccs_ai_alt_text_enqueue_scripts($hook) {
    ...
    wp_enqueue_script(
        ...
        ['jquery'],
        defined('CCS_THEME_VERSION') ? CCS_THEME_VERSION : '1.0',
        true
    );
    wp_localize_script('cta-ai-alt-text', 'ctaAiAltText', [
        'nonce'   => wp_create_nonce('ccs_generate_ai_alt_text'),
        'ajaxurl' => admin_url('admin-ajax.php'),
        'label'   => __('Generate with AI', 'ccs-theme'),
        'loading' => __('Generating…', 'ccs-theme'),
        'error'   => __('Could not generate. Check Settings → AI Assistant for API keys.', 'ccs-theme'),
    ]);
}
add_action('admin_enqueue_scripts', 'ccs_ai_alt_text_enqueue_scripts', 20);
