 ...
 */
function ccs_add_search_console_menu() {
    add_submenu_page(
        'cta-seo',
        'Search Console',
        'Search Console',
        ...
        'ccs_search_console_page'
    );
}
add_action('admin_menu', 'ccs_add_search_console_menu', 25);

/**
 * Search Console admin page
 */
function ccs_search_console_page() {
    // Check if API is configured
    $api_configured = get_option('ccs_gsc_access_token');
    $property_url = get_option('ccs_gsc_property_url', home_url());
    
    // Handle OAuth callback
    if (isset($_GET['code']) && isset($_GET['state']) && wp_verify_nonce($_GET['state'], 'ccs_gsc_oauth')) {
        $code = sanitize_text_field($_GET['code']);
        $token = ccs_gsc_exchange_code_for_token($code);
        
        if ($token && !is_wp_error($token)) {
            update_option('ccs_gsc_access_token', $token);
            update_option('ccs_gsc_token_expires', time() + 3600); // 1 hour
    ...
    // Handle disconnect
    if (isset($_POST['disconnect_gsc']) && check_admin_referer('ccs_disconnect_gsc')) {
        delete_option('ccs_gsc_access_token');
        delete_option('ccs_gsc_token_expires');
        delete_option('ccs_gsc_refresh_token');
        delete_option('ccs_gsc_property_url');
        $api_configured = false;
        echo '<div class="notice notice-success"><p>Disconnected from Search Console.</p></div>';
    }
    
    // Handle property URL save
    if (isset($_POST['save_property_url']) && check_admin_referer('ccs_save_property_url')) {
        $url = esc_url_raw($_POST['property_url']);
        update_option('ccs_gsc_property_url', $url);
        $property_url = $url;
    ...
    <div class="wrap">
        ...
        <div class="card" style="max-width: 800px;">
            ...
            <form method="post">
                <?php wp_nonce_field('ccs_gsc_setup'); ?>
            ...
            if (isset($_POST['save_gsc_credentials']) && check_admin_referer('ccs_gsc_setup')) {
                $client_id = sanitize_text_field($_POST['gsc_client_id']);
                $client_secret = sanitize_text_field($_POST['gsc_client_secret']);
                
                if (!empty($client_id) && !empty($client_secret)) {
                    update_option('ccs_gsc_client_id', $client_id);
                    update_option('ccs_gsc_client_secret', $client_secret);
                    
                    // Generate OAuth URL
                    $redirect_uri = admin_url('admin.php?page=cta-seo-search-console');
                    $state = wp_create_nonce('ccs_gsc_oauth');
        ...
        <div class="card" style="margin-bottom: 20px;">
            ...
            <form method="post" style="margin-top: 15px;">
                <?php wp_nonce_field('ccs_disconnect_gsc'); ?>
        ...
        <form method="post" style="margin-bottom: 20px;">
            <?php wp_nonce_field('ccs_save_property_url'); ?>
            <table class="form-table">
                <tr>
                    <th scope="row">
        ...
        <div class="card">
            ...
            <?php
            // Try to fetch data
            $performance_data = ccs_gsc_get_performance_data($property_url);
 ...
 */
function ccs_gsc_exchange_code_for_token($code) {
    $client_id = get_option('ccs_gsc_client_id');
    $client_secret = get_option('ccs_gsc_client_secret');
    $redirect_uri = admin_url('admin.php?page=cta-seo-search-console');
    ...
    if (isset($body['access_token'])) {
        if (isset($body['refresh_token'])) {
            update_option('ccs_gsc_refresh_token', $body['refresh_token']);
        }
 ...
 */
function ccs_gsc_get_performance_data($property_url) {
    $access_token = get_option('ccs_gsc_access_token');
    ...
    $expires = get_option('ccs_gsc_token_expires', 0);
    if (time() >= $expires) {
        // Try to refresh token
        $refreshed = ccs_gsc_refresh_token();
 ...
 */
function ccs_gsc_refresh_token() {
    $refresh_token = get_option('ccs_gsc_refresh_token');
    $client_id = get_option('ccs_gsc_client_id');
    $client_secret = get_option('ccs_gsc_client_secret');
    ...
    if (isset($body['access_token'])) {
        update_option('ccs_gsc_access_token', $body['access_token']);
        update_option('ccs_gsc_token_expires', time() + ($body['expires_in'] ?? 3600));
        return $body['access_token'];
    }
