 ...
 */
function ccs_form_submissions_admin_title($admin_title, $title) {
    global $typenow;
...
add_filter('admin_title', 'ccs_form_submissions_admin_title', 10, 2);

/**
 * Admin notice: allow seeding test submissions on demand.
 */
function ccs_form_submissions_seed_test_notice() {
    ...
    if (isset($_GET['ccs_seed_test_submissions'])) {
        $status = sanitize_text_field((string) $_GET['ccs_seed_test_submissions']);
        if ($status === 'seeded') {
    ...
    $url = wp_nonce_url(
        admin_url('admin-post.php?action=ccs_seed_test_submissions'),
        'ccs_seed_test_submissions'
    );
...
add_action('admin_notices', 'ccs_form_submissions_seed_test_notice');

/**
 * Handler: seed test submissions.
 */
function ccs_handle_seed_test_submissions_admin_post() {
    if (!current_user_can('manage_options')) {
        wp_die('You do not have permission to do this.');
    }
    check_admin_referer('ccs_seed_test_submissions');
    ...
    if (!empty($existing)) {
        wp_safe_redirect(admin_url('edit.php?post_type=form_submission&ccs_seed_test_submissions=already'));
        exit;
    }

    if (function_exists('ccs_populate_test_form_submissions')) {
        ...
        wp_safe_redirect(admin_url('edit.php?post_type=form_submission&ccs_seed_test_submissions=' . (!empty($existing_after) ? 'seeded' : 'failed')));
        exit;
    }

    wp_safe_redirect(admin_url('edit.php?post_type=form_submission&ccs_seed_test_submissions=failed'));
    exit;
}
add_action('admin_post_ccs_seed_test_submissions', 'ccs_handle_seed_test_submissions_admin_post');

/**
 * Verify admin context and permissions
 * Use this at the start of all admin-only functions
 */
function ccs_verify_admin_access($capability = 'edit_posts') {
    if (!is_admin()) {
        return false;
    }
 ...
 */
function ccs_form_submission_csv_import_menu() {
    add_submenu_page(
        ...
        'edit_posts',
        'cta-import-csv',
        'ccs_form_submission_csv_import_page'
    );
}
add_action('admin_menu', 'ccs_form_submission_csv_import_menu', 25);

/**
 * Add single submission submenu
 */
function ccs_form_submission_add_single_menu() {
    add_submenu_page(
        'edit.php?post_type=form_submission',
        'Add Submission',
        'Add Submission',
        'edit_posts',
        'cta-add-submission',
        'ccs_form_submission_add_single_page'
    );
}
add_action('admin_menu', 'ccs_form_submission_add_single_menu', 26);

/**
 * Handler: add single submission
 */
function ccs_handle_add_single_submission_admin_post() {
    if (!ccs_verify_admin_access('edit_posts')) {
        wp_die('You do not have permission to do this.');
    }
    check_admin_referer('ccs_add_single_submission');
    ...
    if (empty($name) && empty($email)) {
        wp_safe_redirect(admin_url('edit.php?post_type=form_submission&page=cta-add-submission&ccs_add_submission=missing'));
        exit;
    }
    ...
    if (is_wp_error($post_id)) {
        wp_safe_redirect(admin_url('edit.php?post_type=form_submission&page=cta-add-submission&ccs_add_submission=failed'));
        exit;
    }
    ...
    wp_safe_redirect(admin_url('edit.php?post_type=form_submission&page=cta-add-submission&ccs_add_submission=success&post_id=' . absint($post_id)));
    exit;
}
add_action('admin_post_ccs_add_single_submission', 'ccs_handle_add_single_submission_admin_post');

/**
 * Add submission page
 */
function ccs_form_submission_add_single_page() {
    if (!ccs_verify_admin_access('edit_posts')) {
        wp_die('You do not have permission to access this page.');
    }

    $status = isset($_GET['ccs_add_submission']) ? sanitize_text_field((string) $_GET['ccs_add_submission']) : '';
    $post_id = isset($_GET['post_id']) ? absint($_GET['post_id']) : 0;

    $action_url = wp_nonce_url(
        admin_url('admin-post.php?action=ccs_add_single_submission'),
        'ccs_add_single_submission'
    );
 ...
 * CSV Import page
 */
function ccs_form_submission_csv_import_page() {
    ...
    // Handle file upload
    if (isset($_POST['ccs_import_csv']) && check_admin_referer('ccs_import_csv')) {
        ...
        if ($file_path && file_exists($file_path)) {
            $result = ccs_process_csv_import($file_path, $mapping, $edited_data);
    ...
    $stored_file = get_option('ccs_csv_import_temp_file');
    if ($stored_file && file_exists($stored_file)) {
        $preview_data = ccs_preview_csv($stored_file);
        if ($preview_data) {
            $show_mapping = true;
            $csv_headers = $preview_data['headers'];
            $csv_preview = $preview_data['preview'];
            $csv_all_rows = $preview_data['all_rows'] ?? [];
            update_option('ccs_csv_import_all_data', $csv_all_rows, false);
        }
    }
    
    if (isset($_POST['ccs_preview_csv']) && isset($_FILES['csv_file']) && check_admin_referer('ccs_preview_csv')) {
            ...
            if ($file_ext !== 'csv') {
                ...
                if (isset($uploaded_file['error'])) {
                    ...
                    if (filesize($uploaded_file['file']) > 10 * 1024 * 1024) {
                        ...
                        $preview_data = ccs_preview_csv($uploaded_file['file']);
            if ($preview_data) {
                $show_mapping = true;
                $csv_headers = $preview_data['headers'];
                $csv_preview = $preview_data['preview'];
                $csv_all_rows = $preview_data['all_rows'] ?? [];
                            ...
                            update_option('ccs_csv_import_temp_file', $uploaded_file['file'], false);
    ...
    <div class="wrap">
        ...
        <div class="cta-csv-upload-container">
            ...
            <form method="post" enctype="multipart/form-data" action="" id="cta-csv-upload-form">
                <?php wp_nonce_field('ccs_preview_csv'); ?>
                ...
                <p class="submit" style="text-align: center; margin-top: 32px;">
                    <button type="submit" name="ccs_preview_csv" class="button button-primary button-large" style="display: none;" id="preview-btn">
                        Preview & Map Columns
                    </button>
                </p>
            </form>
        ...
        <div class="postbox">
            ...
            <div class="inside">
                ...
                <form method="post" enctype="multipart/form-data" action="">
                    <?php wp_nonce_field('ccs_import_csv'); ?>
                    <input type="hidden" name="csv_file_path" value="<?php echo esc_attr(get_option('ccs_csv_import_temp_file')); ?>">
                    ...
                    <table class="wp-list-table widefat fixed striped">
                        ...
                        <tbody>
                            ...
                            foreach ($csv_headers as $index => $header) :
                                ...
                                $auto_mapped = ccs_auto_detect_column($header, $sample);
                            ?>
                            <tr>
                ...
                <p class="submit">
                    <button type="submit" name="ccs_import_csv" class="button button-primary">Import Leads</button>
 ...
 */
function ccs_auto_detect_column($header, $sample) {
 ...
 * Preview CSV file
 */
function ccs_preview_csv($file_path) {
    ...
    rewind($handle);
    $delimiter = ccs_detect_csv_delimiter($first_line);
 ...
 */
function ccs_detect_csv_delimiter($line) {
    $delimiters = [',', ';', "\t", '|'];
    $max_count = 0;
    $detected = ',';
 ...
 * Process CSV import
 */
function ccs_process_csv_import($file_path, $mapping, $edited_data = []) {
    // SECURITY: Check user has permission
    if (!current_user_can('edit_posts')) {
    ...
    // Resolve file path
    if (!file_exists($file_path)) {
        $file_path = get_option('ccs_csv_import_temp_file');
    }
    ...
    while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
        ...
        // Create submission
        $result = ccs_create_submission_from_import($data);
    ...
    delete_option('ccs_csv_import_temp_file');
 ...
 */
function ccs_create_submission_from_import($data) {
 ...
 */
function ccs_form_submission_admin_menu_sections() {
    global $submenu;
...
add_action('admin_menu', 'ccs_form_submission_admin_menu_sections', 20);

/**
 * Customize admin columns for Form Submissions
 * Show different columns based on active tab
 */
function ccs_form_submission_admin_columns($columns) {
...
add_filter('manage_form_submission_posts_columns', 'ccs_form_submission_admin_columns');

/**
 * Populate custom admin columns for Form Submissions
 */
function ccs_form_submission_admin_column_content($column, $post_id) {
...
add_action('manage_form_submission_posts_custom_column', 'ccs_form_submission_admin_column_content', 10, 2);

/**
 * Format date column with relative dates
 */
function ccs_form_submission_date_column($date, $post_id, $column_name, $post_type) {
...
add_filter('post_date_column_time', 'ccs_form_submission_date_column', 10, 4);

/**
 * Add bulk actions for form submissions
 * NOTE: Disabled because we use custom HTML bulk actions integrated with tabs
 */
// function ccs_form_submission_bulk_actions($actions) {
//     $actions['bulk_assign'] = 'Assign To';
//     $actions['bulk_status'] = 'Change Follow-up Status';
//     return $actions;
// }
// add_filter('bulk_actions-edit-form_submission', 'ccs_form_submission_bulk_actions');

/**
 * Handle bulk actions for form submissions
 */
function ccs_form_submission_handle_bulk_actions($redirect_to, $action, $post_ids) {
...
add_filter('handle_bulk_actions-edit-form_submission', 'ccs_form_submission_handle_bulk_actions', 10, 3);

/**
 * Add bulk action selectors
 */
function ccs_form_submission_bulk_action_selectors() {
    global $typenow;
...
add_action('admin_footer-edit.php', 'ccs_form_submission_bulk_action_selectors');

/**
 * Show bulk action admin notices
 */
function ccs_form_submission_bulk_action_notices() {
...
add_action('admin_notices', 'ccs_form_submission_bulk_action_notices');

/**
 * Make columns sortable
 */
function ccs_form_submission_sortable_columns($columns) {
...
add_filter('manage_edit-form_submission_sortable_columns', 'ccs_form_submission_sortable_columns');

/**
 * Handle custom column sorting
 */
function ccs_form_submission_custom_orderby($query) {
...
add_action('pre_get_posts', 'ccs_form_submission_custom_orderby');

/**
 * Get count of submissions by status (for quick filter badges)
 * Cached to improve performance
 */
function ccs_get_submission_count_by_status($status) {
    $cache_key = 'ccs_submission_count_' . $status;
 ...
 */
function ccs_get_submission_count_by_tab($tab) {
    $cache_key = 'ccs_submission_tab_count_' . $tab;
    $count = get_transient($cache_key);
 ...
 */
function ccs_clear_submission_count_cache($post_id) {
    if (get_post_type($post_id) === 'form_submission') {
        $statuses = ['new', 'needed', 'booked', 'active'];
        foreach ($statuses as $status) {
            delete_transient('ccs_submission_count_' . $status);
        }
        
        // Clear tab counts too
        $tabs = ['course-enquiries', 'event-bookings', 'newsletter', 'other'];
        foreach ($tabs as $tab) {
            delete_transient('ccs_submission_tab_count_' . $tab);
        }
    }
}
add_action('save_post', 'ccs_clear_submission_count_cache');
add_action('delete_post', 'ccs_clear_submission_count_cache');

/**
 * Add custom styles for form submissions admin
 */
function ccs_form_submission_admin_styles() {
    global $typenow;
...
add_action('admin_head', 'ccs_form_submission_admin_styles');

/**
 * Add clean filter interface with pills
 */
function ccs_form_submission_add_filters() {
    global $typenow;
    ...
    $course_enquiries_count = ccs_get_submission_count_by_tab('course-enquiries');
    $event_bookings_count = ccs_get_submission_count_by_tab('event-bookings');
    $group_enquiries_count = ccs_get_submission_count_by_tab('group-enquiries');
    $newsletter_count = ccs_get_submission_count_by_tab('newsletter');
    $other_count = ccs_get_submission_count_by_tab('other');
...
add_action('restrict_manage_posts', 'ccs_form_submission_add_filters');
...
add_filter('bulk_actions-edit-form_submission', 'ccs_form_submission_bulk_actions', 999);

/**
 * Customize WordPress post status views to show All, Published, and Trash
 */
function ccs_customize_post_status_views($views) {
...
add_filter('views_edit-form_submission', 'ccs_customize_post_status_views', 999);

/**
 * Remove duplicate "Filter" button and "All dates" dropdown
 * WordPress adds these by default, but we already have them
 */
function ccs_remove_duplicate_filters() {
    global $typenow;
...
add_action('admin_head', 'ccs_remove_duplicate_filters');

/**
 * Set default posts per page for form submissions
 */
function ccs_form_submission_screen_options($screen_options, $screen) {
    if ($screen->id === 'edit-form_submission') {
        $screen_options['per_page'] = 50;
    }
    return $screen_options;
}
add_filter('set-screen-option', 'ccs_form_submission_screen_options', 10, 2);

/**
 * Set default posts per page on first load
 */
function ccs_form_submission_default_posts_per_page($per_page, $post_type) {
...
add_filter('edit_posts_per_page', 'ccs_form_submission_default_posts_per_page', 10, 2);

/**
 * Filter submissions by form type and marketing consent
 */
function ccs_form_submission_filter_query($query) {
...
add_action('parse_query', 'ccs_form_submission_filter_query');

/**
 * Add meta boxes for submission details
 */
function ccs_form_submission_add_meta_boxes() {
    ...
    add_meta_box(
        'ccs_form_submission_details',
        'Submission Details',
        'ccs_form_submission_details_wp_callback',
        'form_submission',
        'normal',
        'high'
    );
    ...
    // Activity log
    add_meta_box(
        'ccs_form_submission_activity_log',
        'Activity Log',
        'ccs_form_submission_activity_log_callback',
        'form_submission',
        'normal',
        'default'
    );
    ...
    // Follow-up (side)
    add_meta_box(
        'ccs_form_submission_followup',
        'Follow-up',
        'ccs_form_submission_followup_wp_callback',
        'form_submission',
        'side',
        'high'
    );
}
add_action('add_meta_boxes', 'ccs_form_submission_add_meta_boxes');

/**
 * Metabox: Submission Details (WordPress-native UI)
 */
function ccs_form_submission_details_wp_callback($post) {
    if (!current_user_can('edit_posts')) {
 ...
 */
function ccs_form_submission_followup_wp_callback($post) {
    if (!current_user_can('edit_posts')) {
        echo '<p>You do not have permission to view this content.</p>';
        return;
    }
    ...
    $should_clear_notes = get_transient('ccs_clear_notes_' . $post->ID);
    if ($should_clear_notes) {
        delete_transient('ccs_clear_notes_' . $post->ID);
    ...
    $fb_pixel_id = get_option('ccs_facebook_pixel_id', '');
    $fb_enabled = get_option('ccs_facebook_conversions_api_enabled', 1);
    ...
    if (!empty($fb_pixel_id) && $fb_enabled) :
        ...
        if (!empty($meta_lead_id)) :
            ...
            wp_nonce_field('ccs_send_facebook_conversion_' . $post->ID, 'ccs_send_facebook_conversion_nonce');
 ...
 */
function ccs_form_submission_activity_log_callback($post) {
    if (!current_user_can('edit_posts')) {
        echo '<p>You do not have permission to view this content.</p>';
        return;
    }
 ...
 */
function ccs_add_activity_log($post_id, $type, $message, $details = '') {
 ...
 */
function ccs_form_submission_save_followup($post_id) {
    // Check nonce
    if (!isset($_POST['ccs_form_submission_followup_nonce']) || 
        !wp_verify_nonce($_POST['ccs_form_submission_followup_nonce'], 'ccs_form_submission_followup')) {
        return;
    }
    ...
    if (isset($_POST['followup_status'])) {
        ...
        if ($old_status !== $new_status) {
            if ($new_status) {
                ...
                ccs_add_activity_log($post_id, 'status_changed', 'changed status', "From: {$old_label} → To: {$new_label}");
                
                // Trigger Facebook offline conversion tracking
                do_action('ccs_form_submission_status_changed', $post_id, $old_status, $new_status);
            } else {
    ...
    if (isset($_POST['send_facebook_conversion']) && isset($_POST['ccs_send_facebook_conversion_nonce']) && check_admin_referer('ccs_send_facebook_conversion_' . $post_id, 'ccs_send_facebook_conversion_nonce')) {
        ...
        if (empty($lead_id)) {
            ...
            $result = ccs_send_manual_offline_conversion($post_id, $event_name);
            ...
            if (is_wp_error($result)) {
                ...
                ccs_add_activity_log($post_id, 'facebook_conversion_sent', 'sent Facebook conversion', "Event: {$event_name}");
    ...
    if (isset($_POST['followup_notes'])) {
        ...
        if ($old_notes !== $new_notes) {
            if ($new_notes) {
                ...
                // Log note change
                if (!$old_notes) {
                    ccs_add_activity_log($post_id, 'note_added', 'added a note', wp_trim_words($new_notes, 20));
                } else {
                    ccs_add_activity_log($post_id, 'note_updated', 'updated notes', wp_trim_words($new_notes, 20));
                }
                
                // Set flag to clear notes field after page reload
                set_transient('ccs_clear_notes_' . $post_id, '1', 30);
            } else {
    ...
    if (isset($_POST['assigned_to'])) {
        ...
        if ($old_assigned !== $new_assigned) {
            if ($new_assigned) {
                ...
                ccs_add_activity_log($post_id, 'assigned', 'assigned submission', "From: {$old_name} → To: {$new_name}");
            } else {
                ...
                $old_name = $old_assigned ? ($assignee_names[$old_assigned] ?? $old_assigned) : 'Unassigned';
                ccs_add_activity_log($post_id, 'assigned', 'unassigned submission', "From: {$old_name} → To: Unassigned");
            }
        }
    }
}
add_action('save_post', 'ccs_form_submission_save_followup');

/**
 * Remove default publish box actions (submissions shouldn't be editable)
 */
function ccs_form_submission_remove_publish_actions() {
    // Intentionally left blank: keep the core Publish/Update box so admins can save notes/status.
}
add_action('admin_menu', 'ccs_form_submission_remove_publish_actions');


/**
 * Enqueue CRM inline editing script with localized data
 */
function ccs_form_submission_enqueue_scripts($hook) {
    global $typenow;
    ...
    if ($typenow === 'form_submission' && $hook === 'edit.php') {
        ...
        wp_localize_script('cta-crm-inline-editing', 'ctaCRM', [
            'nonces' => [
                'assignment' => wp_create_nonce('ccs_update_submission_assigned'),
                'status' => wp_create_nonce('ccs_update_submission_status'),
                'notes' => wp_create_nonce('ccs_update_submission_notes'),
                'lead_source' => wp_create_nonce('ccs_update_submission_lead_source'),
                'resend_email' => wp_create_nonce('ccs_resend_submission_email'),
            ],
            ...
            jQuery(document).ready(function($) {
                $(document).on('click', '.cta-resend-email', function(e) {
                    ...
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'ccs_resend_submission_email',
                            post_id: postId,
...
add_action('admin_enqueue_scripts', 'ccs_form_submission_enqueue_scripts');

/**
 * AJAX handler for updating submission status
 */
function ccs_update_submission_status_ajax() {
    check_ajax_referer('ccs_update_submission_status', 'nonce');
...
add_action('wp_ajax_ccs_update_submission_status', 'ccs_update_submission_status_ajax');

/**
 * AJAX handler for updating submission notes
 */
function ccs_update_submission_notes_ajax() {
    check_ajax_referer('ccs_update_submission_notes', 'nonce');
    ...
    if ($notes) {
        ...
        if ($old_notes !== $notes) {
            if (!$old_notes) {
                ccs_add_activity_log($post_id, 'note_added', 'added a note', wp_trim_words($notes, 20));
            } else {
                ccs_add_activity_log($post_id, 'note_updated', 'updated notes', wp_trim_words($notes, 20));
            }
        }
    } else {
        delete_post_meta($post_id, '_submission_followup_notes');
...
add_action('wp_ajax_ccs_update_submission_notes', 'ccs_update_submission_notes_ajax');

/**
 * AJAX handler for updating submission assigned to
 */
function ccs_update_submission_assigned_ajax() {
    check_ajax_referer('ccs_update_submission_assigned', 'nonce');
    ...
    if ($assigned_to) {
        ...
        if ($old_assigned !== $assigned_to) {
            ...
            ccs_add_activity_log($post_id, 'assigned', 'assigned to ' . $assignee_name);
        }
        
        // Send email notification if requested
        if ($send_email) {
            $email_sent = ccs_send_assignment_notification_email($post_id, $assigned_to);
        }
    } else {
        delete_post_meta($post_id, '_submission_assigned_to');
    }
...
add_action('wp_ajax_ccs_update_submission_assigned', 'ccs_update_submission_assigned_ajax');

/**
 * AJAX handler for updating submission lead source
 */
function ccs_update_submission_lead_source_ajax() {
    check_ajax_referer('ccs_update_submission_lead_source', 'nonce');
...
add_action('wp_ajax_ccs_update_submission_lead_source', 'ccs_update_submission_lead_source_ajax');

/**
 * Send email notification when a lead is assigned
 */
function ccs_send_assignment_notification_email($post_id, $assignee) {
    ...
    $to = $assignee_emails[$assignee] ?? (defined('CCS_EMAIL_OFFICE') ? CCS_EMAIL_OFFICE : get_option('admin_email'));
    ...
    $body .= "Best regards,\n";
    $body .= "Continuity of Care Services";
 ...
 */
function ccs_resend_submission_email_ajax() {
    check_ajax_referer('ccs_resend_submission_email', 'nonce');
...
add_action('wp_ajax_ccs_resend_submission_email', 'ccs_resend_submission_email_ajax');
