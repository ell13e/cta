...
function ccs_run_seo_verification() {
    ...
    $schema_check = ccs_verify_schema_markup();
    ...
    // Check 2: Robots.txt
    $robots_check = ccs_verify_robots_txt();
    ...
    // Check 3: Sitemap
    $sitemap_check = ccs_verify_sitemap();
    ...
    // Check 4: Meta Tags
    $meta_check = ccs_verify_meta_tags();
    ...
    $canonical_check = ccs_verify_canonical_urls();
    ...
    $visibility_check = ccs_verify_search_engine_visibility();
    ...
    $permalink_check = ccs_verify_permalink_structure();
    ...
    $trustpilot_check = ccs_verify_trustpilot_config();
 ...
 */
function ccs_verify_schema_markup() {
    $check = [
        'name' => 'Schema Markup',
        'status' => 'pass',
        'issues' => [],
 ...
 * Verify robots.txt
 */
function ccs_verify_robots_txt() {
    $check = [
        'name' => 'Robots.txt',
        'status' => 'pass',
        'issues' => [],
    ...
    if (!has_filter('robots_txt', 'ccs_robots_txt')) {
        $check['status'] = 'error';
...
function ccs_verify_meta_tags() {
    ...
    if (!function_exists('ccs_seo_meta_tags')) {
        $check['status'] = 'error';
 ...
 */
function ccs_fetch_canonical_from_url($url) {
 ...
 */
function ccs_verify_canonical_urls() {
    $check = [
        'name' => 'Canonical URLs',
        'status' => 'pass',
        'issues' => [],
    ...
    if (!function_exists('ccs_clean_canonical_url')) {
        $check['status'] = 'warning';
    ...
    $cleaned = ccs_clean_canonical_url($test_url);
    ...
    foreach ($samples as $sample) {
        ...
        $result = ccs_fetch_canonical_from_url($url);
 ...
 */
function ccs_verify_search_engine_visibility() {
    $check = [
 ...
 */
function ccs_verify_permalink_structure() {
    $check = [
 ...
 */
function ccs_verify_trustpilot_config() {
    $check = [
    ...
    $review_count = ccs_get_theme_option('trustpilot_review_count', '');
 ...
 */
function ccs_handle_seo_fix_actions() {
    if (!isset($_POST['ccs_seo_fix_action']) || !current_user_can('manage_options')) {
        return;
    }
    
    check_admin_referer('ccs_seo_fix_action');
    
    $action = sanitize_text_field($_POST['ccs_seo_fix_action']);
    $success = false;
    $message = '';
    ...
    switch ($action) {
        ...
        case 'fix_trustpilot':
            // Get values from POST (theme reads via ccs_trustpilot_rating / ccs_trustpilot_review_count)
            ...
            if ($rating > 0 && $count > 0) {
                set_theme_mod('ccs_trustpilot_rating', $rating . '/5');
                set_theme_mod('ccs_trustpilot_review_count', (string) $count);
                $success = true;
        ...
        case 'regenerate_sitemap':
            ...
            delete_transient('ccs_sitemap_cache');
            do_action('ccs_regenerate_sitemap');
            $success = true;
            $message = 'Sitemap regenerated successfully!';
            break;
            
        case 'enable_schema_markup':
            update_option('ccs_enable_schema_markup', true);
            $success = true;
            $message = 'Schema markup enabled for all applicable pages!';
            break;
            
        case 'disable_schema_markup':
            update_option('ccs_enable_schema_markup', false);
            $success = true;
        ...
        case 'configure_default_meta':
            ...
            if (!empty($default_title_suffix) && !empty($default_description)) {
                update_option('ccs_default_title_suffix', $default_title_suffix);
                update_option('ccs_default_meta_description', $default_description);
                $success = true;
        ...
        case 'enable_canonical_urls':
            update_option('ccs_enable_canonical_urls', true);
            $success = true;
            $message = 'Canonical URLs enabled!';
            break;
            
        case 'disable_canonical_urls':
            update_option('ccs_enable_canonical_urls', false);
            $success = true;
        ...
        case 'configure_social_meta':
            ...
            if (!empty($og_image)) {
                update_option('ccs_default_og_image', $og_image);
            }
            if (!empty($fb_app_id)) {
                update_option('ccs_facebook_app_id', $fb_app_id);
            }
    ...
    delete_transient('ccs_seo_verification_results');
    
    if ($success) {
        add_settings_error('ccs_seo_verification', 'success', $message, 'success');
    } else {
        add_settings_error('ccs_seo_verification', 'error', $message, 'error');
    }
}
add_action('admin_init', 'ccs_handle_seo_fix_actions');

/**
 * Add SEO verification admin page
 * NOTE: Now in SEO admin section, but keeping function for backward compatibility
 */
function ccs_add_seo_verification_page() {
...
// add_action('admin_menu', 'ccs_add_seo_verification_page');

/**
 * SEO verification page content
 */
function ccs_seo_verification_page() {
    // Run verification
    $results = ccs_run_seo_verification();
    
    // Handle AJAX refresh
    if (isset($_POST['ccs_refresh_verification']) && check_admin_referer('ccs_verification_refresh')) {
        $results = ccs_run_seo_verification();
        // Cache results for 1 hour
        set_transient('ccs_seo_verification_results', $results, HOUR_IN_SECONDS);
    } else {
        // Try to get cached results
        $cached = get_transient('ccs_seo_verification_results');
        if ($cached !== false) {
            $results = $cached;
        }
    }
    ...
    settings_errors('ccs_seo_verification');
    ?>
    <div class="wrap cta-seo-page">
        ...
        <div class="cta-seo-section">
            <div class="cta-seo-actions" style="justify-content: space-between; flex-wrap: wrap; gap: 1rem; margin-bottom: 1rem;">
                ...
                <form method="post" style="margin: 0;">
                    <?php wp_nonce_field('ccs_verification_refresh'); ?>
                    <input type="hidden" name="ccs_refresh_verification" value="1">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($check['status'] !== 'pass') : ?>
                                ...
                                ?>
                                <form method="post" style="margin: 0;">
                                    <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                                    <input type="hidden" name="ccs_seo_fix_action" value="fix_search_visibility">
                                ...
                                <form method="post" style="margin: 0;">
                                    <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                                    <input type="hidden" name="ccs_seo_fix_action" value="fix_permalinks">
                                    <button type="submit" class="button button-small button-primary">
                                ...
                                <form method="post" style="margin: 0;">
                                    <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                                    <input type="hidden" name="ccs_seo_fix_action" value="generate_robots">
                                    <button type="submit" class="button button-small button-primary">
                                ...
                                <form method="post" style="margin: 0;">
                                    <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                                    <input type="hidden" name="ccs_seo_fix_action" value="regenerate_sitemap">
                                    <button type="submit" class="button button-small button-primary">
        ...
        <div id="trustpilot-form" class="cta-seo-section" style="display: none;">
            ...
            <form method="post">
                <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                <input type="hidden" name="ccs_seo_fix_action" value="fix_trustpilot">
        ...
        <div class="cta-seo-section">
            ...
            <?php
            $schema_enabled = get_option('ccs_enable_schema_markup', true);
            $canonical_enabled = get_option('ccs_enable_canonical_urls', true);
            $default_title_suffix = get_option('ccs_default_title_suffix', '| Continuity of Care Services');
            $default_description = get_option('ccs_default_meta_description', '');
            $og_image = get_option('ccs_default_og_image', '');
            ?>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            <form method="post" style="display: inline;">
                                <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                                <input type="hidden" name="ccs_seo_fix_action" value="<?php echo $schema_enabled ? 'disable_schema_markup' : 'enable_schema_markup'; ?>">
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post" style="display: inline;">
                                <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                                <input type="hidden" name="ccs_seo_fix_action" value="<?php echo $canonical_enabled ? 'disable_canonical_urls' : 'enable_canonical_urls'; ?>">
                                <button type="submit" class="button button-small">
        ...
        <div id="meta-config-form" class="cta-seo-section" style="display: none;">
            ...
            <form method="post">
                <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                <input type="hidden" name="ccs_seo_fix_action" value="configure_default_meta">
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">Added to the end of page titles. Example: "| Continuity of Care Services"</p>
                        </td>
                    </tr>
                    <tr>
        ...
        <div id="social-config-form" class="cta-seo-section" style="display: none;">
            ...
            <form method="post">
                <?php wp_nonce_field('ccs_seo_fix_action'); ?>
                <input type="hidden" name="ccs_seo_fix_action" value="configure_social_meta">
                <table class="form-table">
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="fb_app_id" id="fb_app_id" 
                                   value="<?php echo esc_attr(get_option('ccs_facebook_app_id', '')); ?>" 
        ...
        <div class="cta-seo-section">
            ...
            <form method="post" action="<?php echo admin_url('admin-post.php'); ?>">
                <?php wp_nonce_field('ccs_export_verification'); ?>
                <input type="hidden" name="action" value="ccs_export_seo_verification">
 ...
 */
function ccs_export_seo_verification() {
    if (!current_user_can('manage_options')) {
        wp_die('Insufficient permissions');
    }
    
    check_admin_referer('ccs_export_verification');
...
add_action('admin_post_ccs_export_seo_verification', 'ccs_export_seo_verification');
