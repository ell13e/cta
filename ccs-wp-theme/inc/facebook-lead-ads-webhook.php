 ...
 */
function ccs_facebook_lead_ads_register_settings() {
    register_setting('ccs_api_keys_settings', 'ccs_facebook_lead_ads_webhook_enabled', [
        'sanitize_callback' => 'absint',
        'default' => 0,
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_facebook_lead_ads_verify_token', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => wp_generate_password(32, false),
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_facebook_lead_ads_form_type', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => 'meta-lead',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_facebook_app_secret', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_fb_webhook_trusted_proxies', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
}
add_action('admin_init', 'ccs_facebook_lead_ads_register_settings');

/**
 * Admin notice when webhook debug/HTTP bypass constants are defined (dev only, not for production).
 */
function ccs_facebook_webhook_admin_notice_dev_constants() {
...
add_action('admin_notices', 'ccs_facebook_webhook_admin_notice_dev_constants');

/**
 * Schedule daily cleanup of old processed lead IDs (30-day retention).
 */
function ccs_facebook_webhook_schedule_cleanup() {
    if (!wp_next_scheduled('ccs_cleanup_old_fb_processed_leads')) {
        wp_schedule_event(time(), 'daily', 'ccs_cleanup_old_fb_processed_leads');
    }
}
add_action('init', 'ccs_facebook_webhook_schedule_cleanup');

/**
 * Daily cron: remove processed lead entries older than 30 days.
 */
function ccs_cleanup_old_fb_processed_leads() {
    $processed = get_option('ccs_fb_processed_leads', []);
    if (!is_array($processed)) {
        return;
    }
    $processed = ccs_facebook_webhook_normalize_processed_leads($processed);
    ...
    update_option('ccs_fb_processed_leads', $processed, false);
}
 ...
 * @return array
 */
function ccs_facebook_webhook_normalize_processed_leads($processed) {
    if (empty($processed)) {
        return [];
    }
 ...
 */
function ccs_register_facebook_lead_ads_webhook() {
    register_rest_route('cta/v1', '/facebook-lead-ads', [
        'methods' => ['GET', 'POST'],
        'callback' => 'ccs_handle_facebook_lead_ads_webhook',
        'permission_callback' => '__return_true', // Webhook validation handled internally
    ]);
}
add_action('rest_api_init', 'ccs_register_facebook_lead_ads_webhook');
 ...
 */
function ccs_handle_facebook_lead_ads_webhook($request) {
    $method = $request->get_method();

    if ($method === 'GET') {
        return ccs_verify_facebook_webhook($request);
    }
    ...
    $enabled = get_option('ccs_facebook_lead_ads_webhook_enabled', 0);
    if (!$enabled) {
        return new WP_Error('webhook_disabled', 'Facebook Lead Ads webhook is disabled', ['status' => 503]);
    }

    $signature_error = ccs_facebook_webhook_verify_signature($request);
    if (is_wp_error($signature_error)) {
        ccs_facebook_webhook_log_request($request, false, 'signature_invalid');
        return $signature_error;
    }

    $rate_limit_error = ccs_facebook_webhook_check_rate_limit($request);
    if (is_wp_error($rate_limit_error)) {
        ccs_facebook_webhook_log_request($request, false, 'rate_limited');
        return $rate_limit_error;
    }

    return ccs_process_facebook_lead($request);
}
 ...
 * @return WP_REST_Response|WP_Error
 */
function ccs_verify_facebook_webhook($request) {
    ...
    $expected_token = get_option('ccs_facebook_lead_ads_verify_token', '');
 ...
 */
function ccs_facebook_webhook_verify_signature($request) {
    $app_secret = get_option('ccs_facebook_app_secret', '');
    if ($app_secret === '') {
 ...
 */
function ccs_facebook_webhook_check_rate_limit($request) {
    $ip = ccs_facebook_webhook_client_ip();
    $key = 'ccs_fb_webhook_rl_' . md5($ip);
    $window = 60; // seconds
    $max_per_window = 60;
 ...
 * @return string
 */
function ccs_facebook_webhook_client_ip() {
    $remote_addr = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '';
    $trusted = get_option('ccs_fb_webhook_trusted_proxies', '');
 ...
 */
function ccs_facebook_webhook_log_request($request, $success, $note = '') {
    $log = get_option('ccs_fb_webhook_request_log', []);
    if (!is_array($log)) {
        $log = [];
    }
    $entry = [
    ...
    update_option('ccs_fb_webhook_request_log', $log, false);
}
 ...
 */
function ccs_facebook_webhook_validate_payload($body) {
    $errors = [];
    $entries = [];
 ...
 * @return WP_REST_Response|WP_Error
 */
function ccs_process_facebook_lead($request) {
    $body = $request->get_json_params();
    $validated = ccs_facebook_webhook_validate_payload($body ?: []);

    if (empty($validated['entries']) && !empty($validated['errors'])) {
        ccs_facebook_webhook_log_request($request, false, 'invalid_payload');
    ...
    $processed_leads = get_option('ccs_fb_processed_leads', []);
    if (!is_array($processed_leads)) {
        $processed_leads = [];
    }
    $processed_leads = ccs_facebook_webhook_normalize_processed_leads($processed_leads);
    ...
    foreach ($validated['entries'] as $item) {
        ...
        $lead_details = ccs_fetch_facebook_lead_details($leadgen_id);
        if (is_wp_error($lead_details)) {
            $errors[] = 'Failed to fetch lead details: ' . $lead_details->get_error_message();
            continue;
        }

        $adset_info = ccs_fetch_facebook_adset_from_lead($lead_data, $lead_details);
        ...
        $parsed_data = ccs_parse_adset_name($adset_name);
        $submission_id = ccs_create_submission_from_facebook_lead($lead_data, $lead_details, $parsed_data);
        ...
        update_option('ccs_fb_processed_leads', $processed_leads, false);
        $processed_ids = array_column($processed_leads, 'id');
        $processed++;
    }

    ccs_facebook_webhook_log_request($request, true, $processed > 0 ? 'processed_' . $processed : 'no_leads');
 ...
 */
function ccs_fetch_facebook_lead_details($leadgen_id) {
    $access_token = get_option('ccs_facebook_access_token', '');
 ...
 */
function ccs_fetch_facebook_adset_from_lead($lead_data, $lead_details) {
    $access_token = get_option('ccs_facebook_access_token', '');
 ...
 * Format: ccs_COURSENAME_DATE (e.g., ccs_EPFA_27.01.2026)
 * 
 * Note: For upcoming courses ads (ccs_upcoming-courses_DATE), returns empty
 ...
 */
function ccs_parse_adset_name($adset_name) {
    $parsed = [
        'course_name' => '',
        'date' => '',
    ];
    ...
    if (preg_match('/^ccs_upcoming-courses_/i', $adset_name)) {
        return $parsed;
    }
    
    $name = preg_replace('/^ccs_/i', '', $adset_name);
 ...
 */
function ccs_parse_booking_quantity($value) {
    if (empty($value)) {
 ...
 */
function ccs_find_course_event_by_course_and_date($course_name, $event_date) {
    if (empty($course_name) || empty($event_date)) {
        return false;
    }
    
    // First find the course
    $course_id = ccs_find_course_by_name($course_name);
    if (!$course_id) {
        return false;
    }
 ...
 */
function ccs_find_course_by_name($course_name) {
 ...
 * @return int|false Course event post ID or false if not found
 */
function ccs_find_course_event_by_text($event_text) {
    if (empty($event_text)) {
        return false;
    }
 ...
 */
function ccs_create_submission_from_facebook_lead($lead_data, $lead_details, $parsed_data = []) {
        ...
        switch ($field_name) {
            ...
            default:
                ...
                if (stripos($field_name, 'event') !== false || 
                    ...
                    $course_event_id = ccs_find_course_event_by_text($field_value);
                    ...
                    $delegates = ccs_parse_booking_quantity($field_value);
                    if ($delegates > 0) {
    ...
    if (!empty($parsed_data['course_name']) && !empty($parsed_data['date'])) {
        $course_event_id = ccs_find_course_event_by_course_and_date($parsed_data['course_name'], $parsed_data['date']);
        ...
        if ($course_event_id) {
            ...
            $course_id = ccs_find_course_by_name($parsed_data['course_name']);
    ...
    $form_type = get_option('ccs_facebook_lead_ads_form_type', 'meta-lead');
    ...
    if (class_exists('\\CCS\\Repositories\\FormSubmissionRepository')) {
        ...
        $submission_id = ccs_save_form_submission($submission_data, $form_type, false, '');
    }
 ...
 */
function ccs_facebook_lead_ads_webhook_settings_fields() {
    $enabled = get_option('ccs_facebook_lead_ads_webhook_enabled', 0);
    $verify_token = get_option('ccs_facebook_lead_ads_verify_token', '');
    
    if (empty($verify_token)) {
        $verify_token = wp_generate_password(32, false);
        update_option('ccs_facebook_lead_ads_verify_token', $verify_token);
    }
    ...
    <div class="cta-api-keys-section">
        ...
        <table class="form-table">
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_lead_ads_webhook_enabled">Enable Webhook</label>
                </th>
                <td>
                    <label>
                        <input type="checkbox" 
                               id="ccs_facebook_lead_ads_webhook_enabled" 
                               name="ccs_facebook_lead_ads_webhook_enabled" 
                               value="1" 
            ...
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_app_secret">App Secret</label>
                </th>
                <td>
                    <input type="password" 
                           id="ccs_facebook_app_secret" 
                           name="ccs_facebook_app_secret" 
                           value="" 
                           class="regular-text"
                           autocomplete="off">
                    <p class="description">
            ...
            <tr>
                <th scope="row">
                    <label for="ccs_fb_webhook_trusted_proxies">Trusted proxy IPs</label>
                </th>
                <td>
                    <input type="text" 
                           id="ccs_fb_webhook_trusted_proxies" 
                           name="ccs_fb_webhook_trusted_proxies" 
                           value="<?php echo esc_attr(get_option('ccs_fb_webhook_trusted_proxies', '')); ?>" 
                           class="large-text"
            ...
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_lead_ads_verify_token">Verify Token</label>
                </th>
                <td>
                    <input type="text" 
                           id="ccs_facebook_lead_ads_verify_token" 
                           name="ccs_facebook_lead_ads_verify_token" 
                           value="<?php echo esc_attr($verify_token); ?>" 
                           class="regular-text"
                           readonly>
                    <input type="hidden" name="ccs_facebook_lead_ads_generate_token" id="ccs_facebook_lead_ads_generate_token" value="0">
                    <button type="button" class="button" onclick="document.getElementById('ccs_facebook_lead_ads_verify_token').select(); document.execCommand('copy'); this.textContent='Copied!'; setTimeout(() => this.textContent='Copy', 2000);">Copy</button>
                    <button type="button" class="button" onclick="if(confirm('Generate a new verify token? The old token will no longer work with Facebook. You will need to update the webhook in Facebook with the new token.')) { document.getElementById('ccs_facebook_lead_ads_generate_token').value = '1'; var form = this.closest('form'); if(form) { form.submit(); } }">Generate New Token</button>
                    <p class="description">
            ...
            <tr>
                <th scope="row">
                    <label for="ccs_facebook_lead_ads_form_type">Form Type</label>
                </th>
                <td>
                    <input type="text" 
                           id="ccs_facebook_lead_ads_form_type" 
                           name="ccs_facebook_lead_ads_form_type" 
