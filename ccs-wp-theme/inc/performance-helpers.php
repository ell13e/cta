 ...
 */
if (get_option('ccs_disable_emojis', false)) {
 ...
 */
if (get_option('ccs_disable_embeds', false)) {
 ...
 */
if (get_option('ccs_post_revision_limit', false)) {
    $limit = absint(get_option('ccs_post_revision_limit', 5));
 ...
 */
function ccs_get_performance_recommendations() {
    $recommendations = [];
    
    // Check 1: Image optimization
    $image_check = ccs_check_image_optimization();
    ...
    $lazy_check = ccs_check_lazy_loading();
    ...
    $minify_check = ccs_check_minification();
 ...
 */
function ccs_check_image_optimization() {
    $result = [
        'status' => 'good',
        'issues' => [],
        'actions' => [],
 ...
 */
function ccs_check_lazy_loading() {
    $result = [
        'status' => 'good',
        'issues' => [],
        'actions' => [],
    ...
    if (!has_filter('wp_get_attachment_image_attributes', 'ccs_lazy_load_images')) {
        $result['status'] = 'warning';
 ...
 */
function ccs_check_minification() {
    $result = [
        'status' => 'warning',
        'issues' => [],
        'actions' => [],
 ...
 */
function ccs_get_core_web_vitals_targets() {
    return [
        'lcp' => [
 ...
 */
function ccs_add_performance_dashboard_widget() {
    wp_add_dashboard_widget(
        'ccs_performance_widget',
        'Performance Optimization',
        'ccs_performance_dashboard_widget_content'
    );
}
add_action('wp_dashboard_setup', 'ccs_add_performance_dashboard_widget');

/**
 * Performance dashboard widget content
 */
function ccs_performance_dashboard_widget_content() {
    $recommendations = ccs_get_performance_recommendations();
    $targets = ccs_get_core_web_vitals_targets();
 ...
 */
function ccs_add_performance_admin_page() {
    // Moved to SEO menu - registration handled in seo-admin.php
    // This function kept for backward compatibility
}
// Removed: add_action('admin_menu', 'ccs_add_performance_admin_page');

/**
 * Handle performance optimization actions
 */
function ccs_handle_performance_actions() {
    if (!isset($_POST['ccs_performance_action']) || !current_user_can('manage_options')) {
        return;
    }
    
    check_admin_referer('ccs_performance_action');
    
    $action = sanitize_text_field($_POST['ccs_performance_action']);
    $success = false;
    $message = '';
    
    switch ($action) {
        case 'optimize_database':
            $success = ccs_optimize_database();
            $message = $success ? 'Database optimized successfully!' : 'Database optimization failed.';
            break;
            
        case 'clear_transients':
            $success = ccs_clear_expired_transients();
            $message = $success ? 'Expired transients cleared!' : 'Failed to clear transients.';
            break;
            
        case 'generate_htaccess':
            $success = ccs_generate_performance_htaccess();
            $message = $success ? 'Browser caching rules added to .htaccess!' : 'Failed to update .htaccess. Check file permissions.';
            break;
            
        case 'disable_emojis':
            update_option('ccs_disable_emojis', true);
            $success = true;
            $message = 'WordPress emojis disabled!';
            break;
            
        case 'enable_emojis':
            delete_option('ccs_disable_emojis');
            $success = true;
            $message = 'WordPress emojis enabled!';
            break;
            
        case 'disable_embeds':
            update_option('ccs_disable_embeds', true);
            $success = true;
            $message = 'WordPress embeds disabled!';
            break;
            
        case 'enable_embeds':
            delete_option('ccs_disable_embeds');
            $success = true;
        ...
        case 'limit_revisions':
            ...
            update_option('ccs_post_revision_limit', $limit);
            $success = true;
            $message = "Post revisions limited to {$limit}!";
            break;
            
        case 'enable_auto_optimization':
            if (!wp_next_scheduled('ccs_auto_database_cleanup')) {
                // Schedule for Sunday at 3 AM
                $timestamp = strtotime('next Sunday 3:00 AM');
                wp_schedule_event($timestamp, 'weekly', 'ccs_auto_database_cleanup');
                $success = true;
        ...
        case 'disable_auto_optimization':
            $timestamp = wp_next_scheduled('ccs_auto_database_cleanup');
            if ($timestamp) {
                wp_unschedule_event($timestamp, 'ccs_auto_database_cleanup');
                $success = true;
    ...
    if ($success) {
        add_settings_error('ccs_performance', 'success', $message, 'success');
    } else {
        add_settings_error('ccs_performance', 'error', $message, 'error');
    }
}
add_action('admin_init', 'ccs_handle_performance_actions');

/**
 * Register custom cron schedule for weekly optimization
 */
function ccs_add_weekly_cron_schedule($schedules) {
...
add_filter('cron_schedules', 'ccs_add_weekly_cron_schedule');

/**
 * Hook the database optimization function to the cron event
 */
add_action('ccs_auto_database_cleanup', 'ccs_optimize_database');

/**
 * Clean up scheduled event on theme deactivation
 */
function ccs_cleanup_scheduled_events() {
    $timestamp = wp_next_scheduled('ccs_auto_database_cleanup');
    if ($timestamp) {
        wp_unschedule_event($timestamp, 'ccs_auto_database_cleanup');
    }
}
register_deactivation_hook(__FILE__, 'ccs_cleanup_scheduled_events');

/**
 * Optimize database tables
 */
function ccs_optimize_database() {
    global $wpdb;
 ...
 */
function ccs_clear_expired_transients() {
    global $wpdb;
 ...
 */
function ccs_generate_performance_htaccess() {
 ...
 */
function ccs_performance_admin_page() {
    $targets = ccs_get_core_web_vitals_targets();
    
    // Get current settings
    $emojis_disabled = get_option('ccs_disable_emojis', false);
    $embeds_disabled = get_option('ccs_disable_embeds', false);
    $revision_limit = get_option('ccs_post_revision_limit', 5);
    ...
    settings_errors('ccs_performance');
    ...
    <div class="wrap">
        ...
        <div class="card" style="margin-top: 20px;">
            ...
            <form method="post" style="display: inline;">
                <?php wp_nonce_field('ccs_performance_action'); ?>
                <input type="hidden" name="ccs_performance_action" value="optimize_database">
            ...
            <form method="post" style="display: inline; margin-left: 10px;">
                <?php wp_nonce_field('ccs_performance_action'); ?>
                <input type="hidden" name="ccs_performance_action" value="clear_transients">
        ...
        <div class="card" style="margin-top: 20px;">
            ...
            <form method="post">
                <?php wp_nonce_field('ccs_performance_action'); ?>
                <input type="hidden" name="ccs_performance_action" value="generate_htaccess">
                <button type="submit" class="button button-primary">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            <form method="post" style="display: inline;">
                                <?php wp_nonce_field('ccs_performance_action'); ?>
                                <input type="hidden" name="ccs_performance_action" value="<?php echo $emojis_disabled ? 'enable_emojis' : 'disable_emojis'; ?>">
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post" style="display: inline;">
                                <?php wp_nonce_field('ccs_performance_action'); ?>
                                <input type="hidden" name="ccs_performance_action" value="<?php echo $embeds_disabled ? 'enable_embeds' : 'disable_embeds'; ?>">
                                <button type="submit" class="button button-small">
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post" style="display: inline;">
                                <?php wp_nonce_field('ccs_performance_action'); ?>
                                <input type="hidden" name="ccs_performance_action" value="limit_revisions">
        ...
        <div class="card" style="margin-top: 20px;">
            ...
            <?php
            $next_scheduled = wp_next_scheduled('ccs_auto_database_cleanup');
            ...
            <form method="post" style="display: inline;">
                <?php wp_nonce_field('ccs_performance_action'); ?>
                <input type="hidden" name="ccs_performance_action" value="<?php echo $auto_enabled ? 'disable_auto_optimization' : 'enable_auto_optimization'; ?>">
