 ...
 */
function ccs_api_keys_register_settings() {
    register_setting('ccs_api_keys_settings', 'ccs_recaptcha_site_key', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_recaptcha_secret_key', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_google_analytics_id', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_google_tag_manager_id', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_facebook_pixel_id', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_google_maps_api_key', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    
    // Eventbrite settings
    register_setting('ccs_api_keys_settings', 'ccs_eventbrite_oauth_token', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_eventbrite_organization_id', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_eventbrite_venue_id', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_eventbrite_auto_upload', [
        'sanitize_callback' => 'absint',
        'default' => 1,
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_eventbrite_auto_publish', [
        'sanitize_callback' => 'absint',
        'default' => 1,
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_eventbrite_auto_sync_spaces', [
        'sanitize_callback' => 'absint',
        'default' => 1,
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_eventbrite_sync_frequency', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => 'hourly',
    ]);
    register_setting('ccs_api_keys_settings', 'ccs_eventbrite_use_ai_description', [
        'sanitize_callback' => 'absint',
        'default' => 1,
    ]);
}
add_action('admin_init', 'ccs_api_keys_register_settings');

/**
 * Add API Keys settings page under Settings menu
 */
function ccs_api_keys_settings_menu() {
    add_options_page(
        'API Keys',
        'API Keys',
        'manage_options',
        'cta-api-keys',
        'ccs_api_keys_settings_page'
    );
}
add_action('admin_menu', 'ccs_api_keys_settings_menu');

/**
 * API Keys Settings page content
 */
function ccs_api_keys_settings_page() {
    if (!current_user_can('manage_options')) {
        return;
    }
    
    if (isset($_POST['ccs_api_keys_submit']) && check_admin_referer('ccs_api_keys_settings')) {
        update_option('ccs_recaptcha_site_key', sanitize_text_field($_POST['ccs_recaptcha_site_key'] ?? ''));
        update_option('ccs_recaptcha_secret_key', sanitize_text_field($_POST['ccs_recaptcha_secret_key'] ?? ''));
        update_option('ccs_google_analytics_id', sanitize_text_field($_POST['ccs_google_analytics_id'] ?? ''));
        update_option('ccs_google_tag_manager_id', sanitize_text_field($_POST['ccs_google_tag_manager_id'] ?? ''));
        update_option('ccs_facebook_pixel_id', sanitize_text_field($_POST['ccs_facebook_pixel_id'] ?? ''));
        update_option('ccs_facebook_access_token', sanitize_text_field($_POST['ccs_facebook_access_token'] ?? ''));
        update_option('ccs_facebook_test_event_code', sanitize_text_field($_POST['ccs_facebook_test_event_code'] ?? ''));
        update_option('ccs_facebook_conversions_api_enabled', isset($_POST['ccs_facebook_conversions_api_enabled']) ? 1 : 0);
        update_option('ccs_facebook_crm_name', sanitize_text_field($_POST['ccs_facebook_crm_name'] ?? 'WordPress'));
        update_option('ccs_facebook_lead_ads_webhook_enabled', isset($_POST['ccs_facebook_lead_ads_webhook_enabled']) ? 1 : 0);
        
        if (isset($_POST['ccs_facebook_lead_ads_generate_token']) && $_POST['ccs_facebook_lead_ads_generate_token'] === '1') {
            update_option('ccs_facebook_lead_ads_verify_token', wp_generate_password(32, false));
        } else {
            $existing_token = get_option('ccs_facebook_lead_ads_verify_token', '');
            if (empty($existing_token)) {
                update_option('ccs_facebook_lead_ads_verify_token', wp_generate_password(32, false));
            }
        }
        
        update_option('ccs_facebook_lead_ads_form_type', sanitize_text_field($_POST['ccs_facebook_lead_ads_form_type'] ?? 'facebook-lead'));
        $app_secret = isset($_POST['ccs_facebook_app_secret']) ? trim((string) $_POST['ccs_facebook_app_secret']) : '';
        if ($app_secret !== '') {
            update_option('ccs_facebook_app_secret', sanitize_text_field($app_secret));
        }
        update_option('ccs_fb_webhook_trusted_proxies', sanitize_text_field($_POST['ccs_fb_webhook_trusted_proxies'] ?? ''));
        update_option('ccs_google_maps_api_key', sanitize_text_field($_POST['ccs_google_maps_api_key'] ?? ''));
        
        update_option('ccs_eventbrite_oauth_token', sanitize_text_field($_POST['ccs_eventbrite_oauth_token'] ?? ''));
        update_option('ccs_eventbrite_organization_id', sanitize_text_field($_POST['ccs_eventbrite_organization_id'] ?? ''));
        update_option('ccs_eventbrite_venue_id', sanitize_text_field($_POST['ccs_eventbrite_venue_id'] ?? ''));
        update_option('ccs_eventbrite_auto_upload', isset($_POST['ccs_eventbrite_auto_upload']) ? 1 : 0);
        update_option('ccs_eventbrite_auto_publish', isset($_POST['ccs_eventbrite_auto_publish']) ? 1 : 0);
        update_option('ccs_eventbrite_auto_sync_spaces', isset($_POST['ccs_eventbrite_auto_sync_spaces']) ? 1 : 0);
        update_option('ccs_eventbrite_sync_frequency', sanitize_text_field($_POST['ccs_eventbrite_sync_frequency'] ?? 'hourly'));
        update_option('ccs_eventbrite_use_ai_description', isset($_POST['ccs_eventbrite_use_ai_description']) ? 1 : 0);
        
        echo '<div class="notice notice-success is-dismissible"><p>Settings saved successfully!</p></div>';
    }
    
    $recaptcha_site_key = get_option('ccs_recaptcha_site_key', '');
    $recaptcha_secret_key = get_option('ccs_recaptcha_secret_key', '');
    $ga_id = get_option('ccs_google_analytics_id', '');
    $gtm_id = get_option('ccs_google_tag_manager_id', '');
    $fb_pixel = get_option('ccs_facebook_pixel_id', '');
    $fb_access_token = get_option('ccs_facebook_access_token', '');
    $fb_test_code = get_option('ccs_facebook_test_event_code', '');
    $fb_enabled = get_option('ccs_facebook_conversions_api_enabled', 1);
    $maps_key = get_option('ccs_google_maps_api_key', '');
    
    $eventbrite_token = get_option('ccs_eventbrite_oauth_token', '');
    $eventbrite_org_id = get_option('ccs_eventbrite_organization_id', '');
    $eventbrite_venue_id = get_option('ccs_eventbrite_venue_id', '');
    $eventbrite_auto_upload = get_option('ccs_eventbrite_auto_upload', 1);
    $eventbrite_auto_publish = get_option('ccs_eventbrite_auto_publish', 1);
    $eventbrite_auto_sync = get_option('ccs_eventbrite_auto_sync_spaces', 1);
    $eventbrite_sync_freq = get_option('ccs_eventbrite_sync_frequency', 'hourly');
    $eventbrite_use_ai = get_option('ccs_eventbrite_use_ai_description', 1);
    $last_sync = get_option('ccs_eventbrite_last_sync');
    ?>
    <div class="wrap">
        ...
        <form method="post" action="">
            <?php wp_nonce_field('ccs_api_keys_settings'); ?>
            ...
            <div class="cta-api-keys-section">
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_recaptcha_site_key">
                        reCAPTCHA Site Key
                    ...
                    <input 
                        type="text" 
                        id="ccs_recaptcha_site_key" 
                        name="ccs_recaptcha_site_key" 
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_recaptcha_secret_key">
                    ...
                    <input 
                        type="password" 
                        id="ccs_recaptcha_secret_key" 
                        name="ccs_recaptcha_secret_key" 
            ...
            <div class="cta-api-keys-section">
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_google_analytics_id">
                        Google Analytics ID
                        <?php if ($ga_id) : ?>
                            <span class="cta-api-key-status configured">✓ Configured</span>
                        <?php else : ?>
                    ...
                    <input 
                        type="text" 
                        id="ccs_google_analytics_id" 
                        name="ccs_google_analytics_id" 
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_google_tag_manager_id">
                    ...
                    <input 
                        type="text" 
                        id="ccs_google_tag_manager_id" 
                        name="ccs_google_tag_manager_id" 
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_facebook_pixel_id">
                        Facebook Pixel ID
                        <?php if ($fb_pixel) : ?>
                            <span class="cta-api-key-status configured">✓ Configured</span>
                        <?php else : ?>
                    ...
                    <input 
                        type="text" 
                        id="ccs_facebook_pixel_id" 
                        name="ccs_facebook_pixel_id" 
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_facebook_access_token">
                    ...
                    <input 
                        type="password" 
                        id="ccs_facebook_access_token" 
                        name="ccs_facebook_access_token" 
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_facebook_test_event_code">Test Event Code (Optional)</label>
                    <input 
                        type="text" 
                        id="ccs_facebook_test_event_code" 
                        name="ccs_facebook_test_event_code" 
                ...
                <div class="cta-api-key-field">
                    <label>
                        <input type="checkbox" 
                               id="ccs_facebook_conversions_api_enabled" 
                               name="ccs_facebook_conversions_api_enabled" 
                               value="1" 
            ...
            if (function_exists('ccs_facebook_lead_ads_webhook_settings_fields')) {
                ccs_facebook_lead_ads_webhook_settings_fields();
            }
            ?>
            ...
            <div class="cta-api-keys-section">
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_google_maps_api_key">
                        Google Maps API Key
                        <?php if ($maps_key) : ?>
                            <span class="cta-api-key-status configured">✓ Configured</span>
                        <?php else : ?>
                    ...
                    <input 
                        type="text" 
                        id="ccs_google_maps_api_key" 
                        name="ccs_google_maps_api_key" 
            ...
            <div class="cta-api-keys-section">
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_eventbrite_oauth_token">
                    ...
                    <input 
                        type="text" 
                        id="ccs_eventbrite_oauth_token" 
                        name="ccs_eventbrite_oauth_token" 
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_eventbrite_organization_id">
                        Organization ID
                    ...
                    <input 
                        type="text" 
                        id="ccs_eventbrite_organization_id" 
                        name="ccs_eventbrite_organization_id" 
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_eventbrite_venue_id">
                    ...
                    <input 
                        type="text" 
                        id="ccs_eventbrite_venue_id" 
                        name="ccs_eventbrite_venue_id" 
                ...
                <div class="cta-api-key-field">
                    <label>
                        <input 
                            type="checkbox" 
                            name="ccs_eventbrite_auto_upload" 
                            value="1" 
                ...
                <div class="cta-api-key-field">
                    <label>
                        <input 
                            type="checkbox" 
                            name="ccs_eventbrite_auto_publish" 
                            value="1" 
                ...
                <div class="cta-api-key-field">
                    <label>
                        <input 
                            type="checkbox" 
                            name="ccs_eventbrite_auto_sync_spaces" 
                            value="1" 
                ...
                <div class="cta-api-key-field">
                    <label for="ccs_eventbrite_sync_frequency">Sync Frequency</label>
                    <select name="ccs_eventbrite_sync_frequency" id="ccs_eventbrite_sync_frequency">
                ...
                <div class="cta-api-key-field">
                    <label>
                        <input 
                            type="checkbox" 
                            name="ccs_eventbrite_use_ai_description" 
                            value="1" 
            ...
            <p class="submit">
                <?php submit_button('💾 Save All API Keys', 'primary button-hero', 'ccs_api_keys_submit', false); ?>
            </p>
        </form>
        ...
        jQuery(document).ready(function($) {
            $('#cta-test-eventbrite-connection').on('click', function() {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'ccs_test_eventbrite_connection',
                        nonce: '<?php echo wp_create_nonce('ccs_test_eventbrite'); ?>'
                    },
...
function ccs_get_recaptcha_site_key() {
    $key = get_option('ccs_recaptcha_site_key', '');
    if (empty($key)) {
        $key = get_theme_mod('ccs_recaptcha_site_key', '');
    }
    return $key;
}
...
function ccs_get_recaptcha_secret_key() {
    $key = get_option('ccs_recaptcha_secret_key', '');
    if (empty($key)) {
        $key = get_theme_mod('ccs_recaptcha_secret_key', '');
    }
    return $key;
}
 ...
 */
function ccs_get_google_analytics_id() {
    return get_option('ccs_google_analytics_id', '');
}

/**
 * Get Google Tag Manager Container ID
 */
function ccs_get_google_tag_manager_id() {
    return get_option('ccs_google_tag_manager_id', '');
}

/**
 * Get Facebook Pixel ID
 */
function ccs_get_facebook_pixel_id() {
    return get_option('ccs_facebook_pixel_id', '');
}
 ...
 */
function ccs_set_recaptcha_keys_if_empty() {
    // Only set if keys are empty
    $current_site_key = get_option('ccs_recaptcha_site_key', '');
    $current_secret_key = get_option('ccs_recaptcha_secret_key', '');
    
    if (empty($current_site_key)) {
        update_option('ccs_recaptcha_site_key', '6Lds4z0sAAAAAN3EIQ0-EvEZB-QE1JLuazfjXKKE');
    }
    
    if (empty($current_secret_key)) {
        update_option('ccs_recaptcha_secret_key', '6Lds4z0sAAAAAPAflnMuBqASawL4zmyNCxKlivIO');
    }
}
add_action('admin_init', 'ccs_set_recaptcha_keys_if_empty');
