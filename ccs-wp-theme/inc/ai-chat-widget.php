...
function ccs_ai_chat_widget() {
    ...
    wp_nonce_field('ccs_ai_chat_nonce', 'ccs_ai_chat_nonce_field');
    ?>
    <style>
        #cta-ai-chat-widget {
            position: fixed;
    ...
    jQuery(document).ready(function($) {
        ...
        function sendMessage(message) {
            ...
            $.ajax({
                ...
                data: {
                    action: 'ccs_ai_chat',
                    nonce: $('#ccs_ai_chat_nonce_field').val(),
                    message: message,
...
add_action('admin_footer', 'ccs_ai_chat_widget');

/**
 * AJAX handler for chat messages
 */
function ccs_ai_chat_ajax() {
    check_ajax_referer('ccs_ai_chat_nonce', 'nonce');
    ...
    $company_knowledge = ccs_get_ai_company_context();
    
    // Build context-aware system prompt
    $system_prompt = "ROLE: Helpful content assistant for Continuity of Care Services (care training, CQC compliance, healthcare topics)
...
REMEMBER: British English, UK sources only, 3-word link anchors - no exceptions unless explicitly requested.";
    ...
    $result = ccs_ai_try_providers($preferred_provider, function($provider, $api_key) use ($system_prompt, $messages) {
        if ($provider === 'groq') return ccs_call_groq_chat($api_key, $system_prompt, $messages);
        if ($provider === 'anthropic') return ccs_call_anthropic_chat($api_key, $system_prompt, $messages);
        return ccs_call_openai_chat($api_key, $system_prompt, $messages);
    });
...
add_action('wp_ajax_ccs_ai_chat', 'ccs_ai_chat_ajax');

/**
 * Call Anthropic API for chat
 */
function ccs_call_anthropic_chat($api_key, $system, $messages) {
 ...
 */
function ccs_call_openai_chat($api_key, $system, $messages) {
 ...
 */
function ccs_call_groq_chat($api_key, $system, $messages) {
    // Prepend system message
    array_unshift($messages, ['role' => 'system', 'content' => $system]);
 ...
 */
function ccs_get_ai_company_context() {
    $context = [];
    
    // Company info
    $context[] = "COMPANY: Continuity of Care Services (CCS)";
