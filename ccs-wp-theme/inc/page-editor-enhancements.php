 ...
 */
function ccs_enable_page_editor() {
...
add_action('init', 'ccs_enable_page_editor');
...
function ccs_add_seo_meta_box() {
    add_meta_box(
        'ccs_page_seo',
        '🔍 SEO & Schema Settings',
        'ccs_page_seo_meta_box_callback',
        'page',
        'normal',
        'high'
    );
 ...
 */
function ccs_page_seo_meta_box_callback($post) {
    wp_nonce_field('ccs_page_seo_nonce', 'ccs_page_seo_nonce');
    
    // Get current values
    $meta_title = ccs_safe_get_field('page_seo_meta_title', $post->ID, '');
    $meta_description = ccs_safe_get_field('page_seo_meta_description', $post->ID, '');
    $schema_type = ccs_safe_get_field('page_schema_type', $post->ID, 'WebPage');
    $primary_keyword = ccs_safe_get_field('page_seo_primary_keyword', $post->ID, '');
    $secondary_keywords = ccs_safe_get_field('page_seo_secondary_keywords', $post->ID, '');
    $noindex = get_post_meta($post->ID, '_ccs_noindex', true);
    $nofollow = get_post_meta($post->ID, '_ccs_nofollow', true);
    $nosnippet = get_post_meta($post->ID, '_ccs_nosnippet', true);
    $max_snippet_length = get_post_meta($post->ID, '_ccs_max_snippet_length', true);
    
    // Calculate SEO score
    $seo_score = ccs_calculate_page_seo_score($post);
    
    // Get auto-generated meta description preview
    $auto_description = ccs_get_meta_description($post);
    ...
    if (get_page_template_slug($post->ID) === 'page-templates/page-about.php') {
        $page_content_for_js = strip_tags(ccs_get_about_page_content_for_seo($post->ID));
    }
    ...
    <div class="cta-seo-meta-box">
        ...
        <div class="cta-seo-main-fields" style="margin-bottom: 20px;">
            ...
            <div style="margin-bottom: 20px;">
                <label for="ccs_meta_title" style="display: block; font-weight: 600; margin-bottom: 6px; font-size: 13px;">
                    Meta Title
            ...
            <!-- Meta Description -->
            <div style="margin-bottom: 20px;">
                <label for="ccs_meta_description" style="display: block; font-weight: 600; margin-bottom: 6px; font-size: 13px;">
                    Meta Description
                    <span id="cta-desc-length" style="color: #646970; font-weight: normal; margin-left: 8px; font-size: 12px;">0/160</span>
                </label>
                <textarea 
                    id="ccs_meta_description" 
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    ...
                    <div>
                        <div style="margin-bottom: 16px;">
                            <label for="ccs_primary_keyword" style="display: block; font-weight: 600; margin-bottom: 6px; font-size: 13px;">
                                Primary Focus Keyword
                            </label>
                            <input 
                                type="text" 
                                id="ccs_primary_keyword" 
                        ...
                        <div style="margin-bottom: 16px;">
                            <label for="ccs_secondary_keywords" style="display: block; font-weight: 600; margin-bottom: 6px; font-size: 13px;">
                                Secondary Keywords
                            </label>
                            <input 
                                type="text" 
                                id="ccs_secondary_keywords" 
                    ...
                    <div>
                        <div style="margin-bottom: 16px;">
                            <label for="ccs_schema_type" style="display: block; font-weight: 600; margin-bottom: 6px; font-size: 13px;">
                                Schema.org Type
                            </label>
                            <select 
                                id="ccs_schema_type" 
                        ...
                        <div style="margin-bottom: 16px;">
                            ...
                            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                                <label for="ccs_max_snippet_length" style="font-size: 13px;">Max snippet length:</label>
                                <input 
                                    type="number" 
                                    id="ccs_max_snippet_length" 
                                    name="ccs_max_snippet_length" 
    ...
    jQuery(document).ready(function($) {
        ...
        function updateCounters() {
            var title = $('#ccs_meta_title').val() || '<?php echo esc_js(get_the_title($post->ID)); ?>';
            var desc = $('#ccs_meta_description').val() || '<?php echo esc_js($auto_description); ?>';
        ...
        // Update on input
        $('#ccs_meta_title, #ccs_meta_description, #ccs_primary_keyword, #ccs_secondary_keywords').on('input', updateCounters);
        ...
        $('#cta-generate-meta-desc').on('click', function() {
            ...
            var primaryKeyword = ($('#ccs_primary_keyword').val() || '').trim();
            var secondaryRaw = ($('#ccs_secondary_keywords').val() || '').split(',').map(function(k) { return k.trim(); }).filter(function(k) { return k.length > 0; });
            ...
            $('#ccs_meta_description').val(generated);
            updateCounters();
        });
        ...
        function updateSEOScore() {
            var title = $('#ccs_meta_title').val() || '<?php echo esc_js(get_the_title($post->ID)); ?>';
            var desc = $('#ccs_meta_description').val() || '<?php echo esc_js($auto_description); ?>';
            var primaryKeyword = $('#ccs_primary_keyword').val().toLowerCase().trim();
            var secondaryKeywords = $('#ccs_secondary_keywords').val().toLowerCase().split(',').map(function(k) { return k.trim(); }).filter(function(k) { return k.length > 0; });
            ...
            var schemaType = $('#ccs_schema_type').val();
            ...
            if (!$('input[name="ccs_noindex"]').is(':checked')) {
                score += 7;
        ...
        $('#ccs_schema_type, #ccs_primary_keyword, #ccs_secondary_keywords, input[name="ccs_noindex"]').on('change', updateSEOScore);
    });
    </script>
 ...
 */
function ccs_get_about_page_content_for_seo($post_id) {
    ...
    $ccs_title     = get_field('ccs_title', $post_id) ?: '';
    $ccs_text      = get_field('ccs_text', $post_id) ?: '';
    ...
    $mission_is_string = is_string($mission_raw) && trim($mission_raw) !== '';
    ...
    if ($mission_is_string) {
        $out .= wp_kses_post(stripslashes($mission_raw));
    } else {
 ...
 */
function ccs_calculate_page_seo_score($post) {
    $score = 0;
    $checks = [];
    ...
    if (get_page_template_slug($post->ID) === 'page-templates/page-about.php') {
        $acf_content = ccs_get_about_page_content_for_seo($post->ID);
        if ($acf_content !== '') {
    ...
    $primary_keyword = strtolower(trim(ccs_safe_get_field('page_seo_primary_keyword', $post->ID, '')));
    ...
    // Secondary keywords in content and subheadings (5 points total)
    $secondary_keywords_field = ccs_safe_get_field('page_seo_secondary_keywords', $post->ID, '');
...
function ccs_save_seo_meta_box($post_id) {
    // Check nonce
    if (!isset($_POST['ccs_page_seo_nonce']) || !wp_verify_nonce($_POST['ccs_page_seo_nonce'], 'ccs_page_seo_nonce')) {
        return;
    }
    ...
    // Save meta title (ccs_safe_update_field order: $field, $value, $post_id)
    if (isset($_POST['page_seo_meta_title'])) {
        ccs_safe_update_field('page_seo_meta_title', sanitize_text_field($_POST['page_seo_meta_title']), $post_id);
    }
    
    // Save meta description
    if (isset($_POST['page_seo_meta_description'])) {
        ccs_safe_update_field('page_seo_meta_description', sanitize_textarea_field($_POST['page_seo_meta_description']), $post_id);
    }
    
    // Save schema type
    if (isset($_POST['page_schema_type'])) {
        ccs_safe_update_field('page_schema_type', sanitize_text_field($_POST['page_schema_type']), $post_id);
    }
    
    // Save keywords
    if (isset($_POST['page_seo_primary_keyword'])) {
        ccs_safe_update_field('page_seo_primary_keyword', sanitize_text_field($_POST['page_seo_primary_keyword']), $post_id);
    }
    if (isset($_POST['page_seo_secondary_keywords'])) {
        ccs_safe_update_field('page_seo_secondary_keywords', sanitize_text_field($_POST['page_seo_secondary_keywords']), $post_id);
    }
    
    // Save noindex/nofollow
    $noindex = isset($_POST['ccs_noindex']) ? '1' : '0';
    $nofollow = isset($_POST['ccs_nofollow']) ? '1' : '0';
    update_post_meta($post_id, '_ccs_noindex', $noindex);
    update_post_meta($post_id, '_ccs_nofollow', $nofollow);
    
    // Save snippet control
    $nosnippet = isset($_POST['ccs_nosnippet']) ? '1' : '0';
    update_post_meta($post_id, '_ccs_nosnippet', $nosnippet);
    $max_snippet = isset($_POST['ccs_max_snippet_length']) ? sanitize_text_field($_POST['ccs_max_snippet_length']) : '';
    if ($max_snippet !== '' && is_numeric($max_snippet)) {
        $max_snippet = (string) max(1, min(320, (int) $max_snippet));
    }
    update_post_meta($post_id, '_ccs_max_snippet_length', $max_snippet);
    
    // Clear sitemap cache when SEO settings change
    ccs_flush_sitemap_cache($post_id, get_post($post_id));
}
add_action('save_post', 'ccs_save_seo_meta_box');

/**
 * Add editor toggle button helper text
 */
function ccs_add_editor_toggle_help() {
    global $post_type;
...
add_action('admin_footer-post.php', 'ccs_add_editor_toggle_help');
add_action('admin_footer-post-new.php', 'ccs_add_editor_toggle_help');

/**
 * Ensure pages support editor
 */
function ccs_ensure_page_editor_support() {
    add_post_type_support('page', 'editor');
    add_post_type_support('page', 'excerpt');
    add_post_type_support('page', 'thumbnail');
}
add_action('init', 'ccs_ensure_page_editor_support');
