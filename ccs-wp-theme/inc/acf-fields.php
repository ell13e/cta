 ...
 */
function ccs_register_acf_fields() {
    ...
    acf_add_local_field_group([
        ...
        'fields' => [
            ...
            [
                ...
                'name' => 'mission_text',
                'type' => 'wysiwyg',
                'instructions' => 'The mission section text on the page comes from here—not from the main editor above. Edit the content below.',
                'tabs' => 'visual',
                'toolbar' => 'full',
                'media_upload' => 0,
                'default_value' => "<p>Continuity of Care Services' ethos reflects our goal: to urge businesses to invest in their staff, and individuals to invest in themselves.</p><p>We position ourselves as <strong>'the external training room'</strong> that becomes part of your organisation. We don't just deliver courses, we partner with you.</p><p>When working with new care providers, we take time to understand your policies and procedures, ensuring our training complements how your organisation operates. We tailor our training to align perfectly with your needs, creating a seamless integration with your existing processes and standards.</p>",
            ],
            [
            ...
            [
                ...
                'name' => 'values',
                'type' => 'message',
                'message' => 'Value cards are edited in the <strong>About – Value cards</strong> meta box below (works without ACF Pro).',
            ],
            ...
            [
                ...
                'name' => 'stats',
                'type' => 'message',
                'message' => 'Statistics are edited in the <strong>About – Statistics</strong> meta box below (works without ACF Pro).',
            ],
    ...
    add_filter('acf/load_value/name=testimonials', function ($value, $post_id, $field) {
        ...
        return ccs_get_page_testimonials($post_id);
    }, 10, 3);
    ...
    acf_add_local_field_group([
        ...
        'fields' => [
            ...
            [
                'key' => 'field_resources_cqc_inspection_ccs_title',
                'label' => 'CTA Title',
                'name' => 'inspection_ccs_title',
                'type' => 'text',
                'default_value' => 'Get Inspection Ready',
            ],
            [
                'key' => 'field_resources_cqc_inspection_ccs_text',
                'label' => 'CTA Description',
                'name' => 'inspection_ccs_text',
                'type' => 'textarea',
                'default_value' => 'Download our comprehensive checklist to ensure your training records meet CQC standards',
                'rows' => 2,
            ],
            [
                'key' => 'field_resources_cqc_inspection_ccs_button_text',
                'label' => 'CTA Button Text',
                'name' => 'inspection_ccs_button_text',
                'type' => 'text',
                'default_value' => 'Download Inspection Readiness Checklist',
            ],
            [
                'key' => 'field_resources_cqc_inspection_ccs_link',
                'label' => 'CTA Button Link',
                'name' => 'inspection_ccs_link',
                'type' => 'page_link',
                'post_type' => ['page', 'ccs_resource'],
...
add_action('acf/init', 'ccs_register_acf_fields');

/**
 * Get default values for page fields based on template or slug
 */
function ccs_get_page_defaults($post_id) {
 ...
 */
function ccs_populate_hero_title($value, $post_id, $field) {
    if ($value !== null && $value !== '') {
        return $value;
    }
    
    if ($field['name'] === 'hero_title') {
        $defaults = ccs_get_page_defaults($post_id);
...
add_filter('acf/load_value/name=hero_title', 'ccs_populate_hero_title', 10, 3);

/**
 * Auto-populate hero_subtitle from page defaults if empty
 */
function ccs_populate_hero_subtitle($value, $post_id, $field) {
    if ($value !== null && $value !== '') {
        return $value;
    }
...
add_filter('acf/load_value/name=hero_subtitle', 'ccs_populate_hero_subtitle', 10, 3);

/**
 * Auto-populate section heading fields from page defaults if empty
 */
function ccs_populate_section_headings($value, $post_id, $field) {
    if ($value !== null && $value !== '') {
        return $value;
    }
...
add_filter('acf/load_value/name=section_how_it_works_title', 'ccs_populate_section_headings', 10, 3);
add_filter('acf/load_value/name=section_testimonials_title', 'ccs_populate_section_headings', 10, 3);
add_filter('acf/load_value/name=section_benefits_title', 'ccs_populate_section_headings', 10, 3);
add_filter('acf/load_value/name=section_benefits_subtitle', 'ccs_populate_section_headings', 10, 3);
add_filter('acf/load_value/name=section_form_title', 'ccs_populate_section_headings', 10, 3);
add_filter('acf/load_value/name=section_form_description', 'ccs_populate_section_headings', 10, 3);
add_filter('acf/load_value/name=section_faq_title', 'ccs_populate_section_headings', 10, 3);

/**
 * Get default content sections for pages based on template
 */
function ccs_get_page_content_sections($post_id) {
    $post = get_post($post_id);
    if (!$post || $post->post_type !== 'page') {
        return [];
    }
 ...
 */
function ccs_get_page_testimonials($post_id) {
    $post = get_post($post_id);
    if (!$post || $post->post_type !== 'page') {
        return [];
    }
 ...
 */
function ccs_get_page_faqs($post_id) {
    $post = get_post($post_id);
    if (!$post || $post->post_type !== 'page') {
        return [];
    }
 ...
 */
function ccs_populate_page_content_sections($value, $post_id, $field) {
    if ($value !== null && $value !== false && !empty($value)) {
        return $value;
    }
    
    if ($field['name'] === 'page_content_sections') {
        $default_sections = ccs_get_page_content_sections($post_id);
...
add_filter('acf/load_value/name=page_content_sections', 'ccs_populate_page_content_sections', 10, 3);

/**
 * Auto-populate testimonials from page defaults if empty
 */
function ccs_populate_page_testimonials($value, $post_id, $field) {
    if ($value !== null && $value !== false && !empty($value)) {
        return $value;
    }
    
    if ($field['name'] === 'testimonials') {
        $default_testimonials = ccs_get_page_testimonials($post_id);
...
add_filter('acf/load_value/name=testimonials', 'ccs_populate_page_testimonials', 10, 3);

/**
 * Auto-populate FAQs from page defaults if empty
 */
function ccs_populate_page_faqs($value, $post_id, $field) {
    if ($value !== null && $value !== false && !empty($value)) {
        return $value;
    }
    
    if ($field['name'] === 'faqs') {
        $default_faqs = ccs_get_page_faqs($post_id);
...
add_filter('acf/load_value/name=faqs', 'ccs_populate_page_faqs', 10, 3);

/**
 * Auto-populate course_intro_paragraph from course_description if empty
 */
function ccs_populate_course_intro($value, $post_id, $field) {
    if ($value !== null && $value !== false && !empty($value)) {
        return $value;
    }
...
add_filter('acf/load_value/name=course_intro_paragraph', 'ccs_populate_course_intro', 10, 3);

/**
 * Auto-populate course_benefits from certificate/accreditation if empty
 */
function ccs_populate_course_benefits($value, $post_id, $field) {
    if ($value !== null && $value !== false && !empty($value)) {
        return $value;
    }
...
add_filter('acf/load_value/name=course_benefits', 'ccs_populate_course_benefits', 10, 3);
 ...
 */
function ccs_populate_faqs_on_activation() {
    ...
    foreach ($all_pages as $page_id) {
        ...
        $default_faqs = ccs_get_page_faqs($page_id);
    ...
    if ($faqs_populated > 0) {
        update_option('ccs_faqs_populated_count', $faqs_populated);
    }
}
add_action('after_switch_theme', 'ccs_populate_faqs_on_activation', 25);

/**
 * Helper function to safely get ACF field with fallback
 */
function ccs_get_field($field_name, $post_id = false, $default = '') {
 ...
 */
function ccs_get_custom_excerpt($post_id = false) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
    ...
    if ($post_type === 'course') {
        $custom_excerpt = ccs_get_field('course_seo_excerpt', $post_id, '');
        if (!empty($custom_excerpt)) {
            return $custom_excerpt;
        }
    } elseif ($post_type === 'course_event') {
        $custom_excerpt = ccs_get_field('event_seo_excerpt', $post_id, '');
        if (!empty($custom_excerpt)) {
            return $custom_excerpt;
        }
    }
 ...
 */
function ccs_filter_excerpt($excerpt, $post = null) {
    if (!$post) {
        $post = get_post();
    }
    ...
    // Check for custom excerpt fields
    if ($post_type === 'course') {
        $custom_excerpt = ccs_get_field('course_seo_excerpt', $post->ID, '');
        if (!empty($custom_excerpt)) {
            return $custom_excerpt;
        }
    } elseif ($post_type === 'course_event') {
        $custom_excerpt = ccs_get_field('event_seo_excerpt', $post->ID, '');
        if (!empty($custom_excerpt)) {
            return $custom_excerpt;
        }
    }
...
add_filter('get_the_excerpt', 'ccs_filter_excerpt', 10, 2);

/**
 * Helper function to get learning outcomes as array
 * Parses the textarea (one outcome per line) into an array
 */
function ccs_get_outcomes($post_id = false) {
    $outcomes_text = ccs_get_field('course_outcomes', $post_id, '');
 ...
 */
function ccs_get_stats($post_id = false) {
    $stats_text = ccs_get_field('about_stats', $post_id, '');
...
/**
 * Legacy compatibility: ccs_get_repeater now works with textarea format
 */
function ccs_get_repeater($field_name, $post_id = false) {
    // Handle outcomes field
    if ($field_name === 'course_outcomes') {
        return ccs_get_outcomes($post_id);
    }
    
    // Handle stats field
    if ($field_name === 'about_stats') {
        return ccs_get_stats($post_id);
    }
 ...
 */
function ccs_add_eventbrite_acf_fields() {
    if (!function_exists('acf_add_local_field_group')) {
        return;
    }
...
add_action('acf/init', 'ccs_add_eventbrite_acf_fields');

/**
 * Add ACF fields for FAQ post type
 */
function ccs_add_faq_acf_fields() {
    if (!function_exists('acf_add_local_field_group')) {
        return;
    }
...
add_action('acf/init', 'ccs_add_faq_acf_fields');

/**
 * Add ACF fields for FAQ Category taxonomy (icon)
 */
function ccs_add_faq_category_acf_fields() {
    if (!function_exists('acf_add_local_field_group')) {
        return;
    }
...
add_action('acf/init', 'ccs_add_faq_category_acf_fields');
