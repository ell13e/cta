...
function ccs_strip_category_base_option($value) {
    if (!(bool) get_option('ccs_strip_category_base_enabled', 1)) {
        return $value;
    }
    return '.';
}
 ...
 */
function ccs_auto_generate_alt_text($attr, $attachment, $size) {
...
add_filter('wp_get_attachment_image_attributes', 'ccs_auto_generate_alt_text', 20, 3);
 ...
 */
function ccs_bulk_add_alt_text() {
    $images = get_posts([
 ...
 */
function ccs_bulk_alt_text_admin_notice() {
    if (isset($_GET['ccs_bulk_alt_done'])) {
        $count = intval($_GET['ccs_bulk_alt_done']);
        ?>
...
add_action('admin_notices', 'ccs_bulk_alt_text_admin_notice');

/**
 * Add bulk alt text tool to admin
 */
function ccs_add_bulk_alt_text_tool() {
    if (isset($_GET['ccs_bulk_alt_text']) && current_user_can('manage_options')) {
        check_admin_referer('ccs_bulk_alt_text');
        $count = ccs_bulk_add_alt_text();
        wp_redirect(admin_url('upload.php?ccs_bulk_alt_done=' . $count));
        exit;
    }
}
add_action('admin_init', 'ccs_add_bulk_alt_text_tool');

/**
 * Add bulk alt text button to media library
 */
function ccs_add_bulk_alt_text_button($views) {
    if (current_user_can('manage_options')) {
        $url = wp_nonce_url(admin_url('upload.php?ccs_bulk_alt_text=1'), 'ccs_bulk_alt_text');
        $views['ccs_bulk_alt'] = '<a href="' . esc_url($url) . '" class="button">Add Alt Text to All Images</a>';
    }
    return $views;
}
add_filter('views_upload', 'ccs_add_bulk_alt_text_button');