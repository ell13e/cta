 ...
 */
function ccs_has_upcoming_events() {
    $today = date('Y-m-d');
 ...
 */
function ccs_populate_test_discounts() {
    // Check if site-wide discount already exists
    $existing_site_wide = get_option('ccs_site_wide_discount_active', false);
    
    if (!$existing_site_wide) {
        // Create a site-wide discount (15% off, expires in 2 months)
        $expiry_date = date('Y-m-d', strtotime('+2 months'));
        update_option('ccs_site_wide_discount_active', true);
        update_option('ccs_site_wide_discount_percentage', 15);
        update_option('ccs_site_wide_discount_label', 'Spring Sale');
        update_option('ccs_site_wide_discount_expiry', $expiry_date);
    }
    
    // Check if discount codes exist
    $existing_codes = ccs_get_discount_codes();
    ...
    if (empty($existing_codes)) {
        ...
        ccs_save_discount_codes($test_codes);
    }
 ...
 */
function ccs_populate_test_course_events() {
    // Check if we already have upcoming events
    if (ccs_has_upcoming_events()) {
        return;
    }
 ...
 */
function ccs_populate_events_on_activation() {
    ccs_populate_test_discounts();
    ccs_populate_test_course_events();
}
add_action('after_switch_theme', 'ccs_populate_events_on_activation');

/**
 * Run on theme update (check version)
 */
function ccs_populate_events_on_update() {
    $stored_version = get_option('ccs_theme_version', '0.0.0');
    $current_version = CCS_THEME_VERSION;
    
    // Only run if version has changed
    if (version_compare($stored_version, $current_version, '<')) {
        ccs_populate_test_discounts();
        ccs_populate_test_course_events();
        update_option('ccs_theme_version', $current_version);
    }
}
add_action('admin_init', 'ccs_populate_events_on_update');
