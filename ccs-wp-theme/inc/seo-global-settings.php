 ...
 */
function ccs_get_title_template($post_type = 'post') {
    $templates = [
    ...
    $custom_template = ccs_safe_get_field('seo_title_template_' . $post_type, 'option', '');
 ...
 */
function ccs_process_title_template($template, $post = null) {
    if (!$post) {
        global $post;
    }
 ...
 */
function ccs_apply_title_template($title_parts) {
    global $post;
    ...
    if (get_post_type() === 'course') {
        ...
        $custom_title = ccs_safe_get_field('event_seo_meta_title', get_the_ID(), '');
    } elseif (get_post_type() === 'post') {
        $custom_title = ccs_safe_get_field('news_meta_title', get_the_ID(), '');
    } elseif (get_post_type() === 'page') {
        $custom_title = ccs_safe_get_field('page_seo_meta_title', get_the_ID(), '');
    }
    ...
    // Apply template
    $template = ccs_get_title_template(get_post_type());
    $title_parts['title'] = ccs_process_title_template($template, $post);
    
    // Ensure max 60 chars
    if (strlen($title_parts['title']) > 60) {
        $title = get_the_title();
        $sitename = get_bloginfo('name');
        $sep = ccs_safe_get_field('seo_title_separator', 'option', '–');
...
add_filter('document_title_parts', 'ccs_apply_title_template', 5);
 ...
 */
function ccs_get_global_robots_meta() {
    return [
...
function ccs_apply_global_robots_meta($robots) {
    $global = ccs_get_global_robots_meta();
 ...
 * - Image alt text
 */
function ccs_test_primary_keyword_only($keyword, $post) {
    $tests = [];
    ...
    $title = get_the_title($post->ID);
    $meta_title = ccs_safe_get_field('page_seo_meta_title', $post->ID, '') ?: $title;
    $meta_desc = ccs_safe_get_field('page_seo_meta_description', $post->ID, '');
 ...
 */
function ccs_test_all_keywords($keywords, $post) {
    $tests = [];
 ...
 */
function ccs_get_schema_template($post_type = 'post') {
    $templates = [
 ...
 */
function ccs_apply_schema_template($post_id) {
    $post_type = get_post_type($post_id);
    
    // Check if schema type is already set
    $existing_schema = ccs_safe_get_field('page_schema_type', $post_id, '');
    if (!empty($existing_schema)) {
        return; // Don't override manual settings
    }
    
    // Apply template
    $template = ccs_get_schema_template($post_type);
    if ($template !== 'None') {
        ccs_safe_update_field('page_schema_type', $template, $post_id);
    }
}
add_action('save_post', 'ccs_apply_schema_template', 10, 1);
 ...
 */
function ccs_configure_sitemap() {
...
add_action('init', 'ccs_configure_sitemap');
 ...
 */
function ccs_noindex_empty_archives($robots) {
    // Check if setting is enabled
    $noindex_empty = ccs_safe_get_field('seo_noindex_empty_archives', 'option', false);
...
add_filter('wp_robots', 'ccs_noindex_empty_archives', 10);
