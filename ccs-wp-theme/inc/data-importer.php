 ...
 * - It only runs once (checks ccs_data_imported option)
 ...
 */
function ccs_get_post_by_title($title, $post_type = 'page') {
    global $wpdb;
...
function ccs_import_data_on_activation() {
    ...
    try {
        ...
        if (function_exists('ccs_import_site_settings')) {
            ccs_import_site_settings();
        }
        
        // Import courses
        $courses_imported = 0;
        if (function_exists('ccs_import_courses')) {
            $courses_imported = ccs_import_courses();
        }
        
        // Import scheduled events
        $events_imported = 0;
        if (function_exists('ccs_import_scheduled_events')) {
            $events_imported = ccs_import_scheduled_events();
        }
        
        // Import course categories
        if (function_exists('ccs_import_course_categories')) {
            ccs_import_course_categories();
        }
        
        // Import news articles
        $news_imported = 0;
        if (function_exists('ccs_import_news_articles')) {
            $news_imported = ccs_import_news_articles();
        }
        
        // Import team members
        $team_imported = 0;
        if (function_exists('ccs_import_team_members')) {
            $team_imported = ccs_import_team_members();
        }
        
        // Create static pages
        $pages_created = 0;
        if (function_exists('ccs_create_static_pages')) {
            $pages_created = ccs_create_static_pages();
        }
        
        // Parse and store reviews
        $reviews_parsed = 0;
        if (function_exists('ccs_parse_and_store_reviews')) {
            $reviews_parsed = ccs_parse_and_store_reviews();
        }
        
        // Populate new course fields for existing courses
        $courses_populated = 0;
        if (function_exists('ccs_populate_course_expanded_fields')) {
            $courses_populated = ccs_populate_course_expanded_fields();
        }
        
        // Populate Safeguarding course with SEO-optimized content
        if (function_exists('ccs_populate_safeguarding_course_content')) {
            ccs_populate_safeguarding_course_content();
        }
        
        // Set up default WordPress settings
        if (function_exists('ccs_configure_wordpress_settings')) {
            ccs_configure_wordpress_settings();
        }
        
        // Mark as imported
        update_option('ccs_data_imported', true);
        update_option('ccs_data_import_date', current_time('mysql'));
        update_option('ccs_courses_imported_count', $courses_imported);
        update_option('ccs_events_imported_count', $events_imported);
        update_option('ccs_news_imported_count', $news_imported);
        update_option('ccs_team_imported_count', $team_imported);
        update_option('ccs_pages_created_count', $pages_created);
        update_option('ccs_reviews_parsed_count', $reviews_parsed);
        update_option('ccs_courses_expanded_populated_count', $courses_populated);
        
        // Add admin notice
        set_transient('ccs_import_complete', [
        ...
        update_option('ccs_data_import_attempted', true);
    }
}
add_action('after_switch_theme', 'ccs_import_data_on_activation');

/**
 * Show admin notice after import
 */
function ccs_show_import_notice() {
    $import_data = get_transient('ccs_import_complete');
    
    if ($import_data) {
        ?>
        <div class="notice notice-success is-dismissible">
            <p><strong>🎉 Continuity of Care Services Theme Ready!</strong></p>
        ...
        <?php
        delete_transient('ccs_import_complete');
    }
}
add_action('admin_notices', 'ccs_show_import_notice');

/**
 * Import courses from JSON data
 */
function ccs_import_courses() {
    $courses_data = ccs_get_courses_data();
    ...
    foreach ($courses_data as $course_title => $course) {
        ...
        if (!empty($course['image'])) {
            ccs_set_course_featured_image($post_id, $course['image'], $course_title);
        }
 ...
 */
function ccs_populate_courses_from_json($course_titles) {
    $courses_data = ccs_get_courses_data();
    ...
    foreach ($course_titles as $course_title) {
        $result = ccs_populate_single_course_from_json($course_title);
        $results[] = $result;
 ...
 */
function ccs_populate_single_course_from_json($course_title) {
    $courses_data = ccs_get_courses_data();
 ...
 */
function ccs_import_scheduled_events() {
    $events_data = ccs_get_scheduled_events_data();
    ...
    foreach ($events_data as $event) {
        ...
        $course = ccs_get_post_by_title($event['title'], 'course');
        
        // If exact match fails, try fuzzy matching (handles variations like "&" vs "and")
        if (!$course && function_exists('ccs_find_course_by_name')) {
            $course_id = ccs_find_course_by_name($event['title']);
            if ($course_id) {
        ...
        $date = ccs_parse_event_date($event['date']);
        ...
        $existing = ccs_get_post_by_title($event_title, 'course_event');
        ...
        if (function_exists('update_field')) {
            ...
            if (!empty($event['time'])) {
                $times = ccs_parse_event_time($event['time']);
                if ($times) {
    ...
    update_option('ccs_events_import_last_result', [
        'imported' => $imported,
 ...
 * Import course categories
 */
function ccs_import_course_categories() {
    $categories = [
        'Core Care Skills' => [
 ...
 */
function ccs_import_news_articles() {
    $json_file = CCS_THEME_DIR . '/data/news-articles.json';
    ...
    foreach ($articles as $article) {
        ...
        $existing = ccs_get_post_by_title($article['title'], 'post');
 ...
 */
function ccs_page_exists_by_slug_any_status($slug) {
    if (!$slug) {
        return null;
    }
    $posts = get_posts([
 ...
 */
function ccs_create_static_pages() {
    $pages = [
        [
            'title' => 'Home',
            'slug' => 'home',
    ...
    foreach ($pages as $page_data) {
        ...
        if (!$existing) {
            $existing = ccs_page_exists_by_slug_any_status($page_data['slug']);
        }
 ...
 */
function ccs_maybe_seed_missing_static_pages() {
    ...
    if (get_option('ccs_static_pages_seed_v3') === '1') {
        return;
    }
    // Mark as run immediately so we don't run again on every admin page load
    update_option('ccs_static_pages_seed_v3', '1', false);
    if (function_exists('ccs_create_static_pages')) {
        ccs_create_static_pages();
    }
}
add_action('admin_init', 'ccs_maybe_seed_missing_static_pages', 20);
 ...
 */
function ccs_create_missing_static_page_on_404() {
    if (!is_404()) {
        return;
    }
    ...
    if (function_exists('ccs_page_exists_by_slug_any_status') && ccs_page_exists_by_slug_any_status($slug)) {
        return;
    }
...
add_action('template_redirect', 'ccs_create_missing_static_page_on_404', 0);

/**
 * Get courses data from embedded JSON
 */
function ccs_get_courses_data() {
    // Embedded course data (from courses-database.json)
    $json_file = CCS_THEME_DIR . '/data/courses-database.json';
 ...
 */
function ccs_get_scheduled_events_data() {
    $json_file = CCS_THEME_DIR . '/data/scheduled-courses.json';
 ...
 */
function ccs_parse_event_date($date_string) {
 ...
 */
function ccs_parse_event_time($time_string) {
 ...
 */
function ccs_set_course_featured_image($post_id, $image_path, $course_title) {
 ...
 */
function ccs_add_import_admin_page() {
    add_submenu_page(
        'tools.php',
        'Import CTA Data',
        'Import CTA Data',
        'manage_options',
        'cta-import-data',
        'ccs_import_admin_page_content'
    );
}
add_action('admin_menu', 'ccs_add_import_admin_page');

/**
 * Enqueue styles and scripts only on Import CTA Data admin page
 */
function ccs_import_admin_enqueue_scripts($hook) {
    if ($hook !== 'tools_page_cta-import-data') {
        return;
    }
    $version = defined('CCS_THEME_VERSION') ? CCS_THEME_VERSION : '1.0.0';
    wp_enqueue_style(
        'ccs-import-admin',
        get_template_directory_uri() . '/assets/css/ccs-import-admin.css',
        [],
        $version
    );
    wp_enqueue_script(
        'ccs-import-admin',
        get_template_directory_uri() . '/assets/js/ccs-import-admin.js',
        [],
        $version,
        true
    );
}
add_action('admin_enqueue_scripts', 'ccs_import_admin_enqueue_scripts');
 ...
 */
function ccs_fix_cpt_page_slug_conflict_manual() {
    $changed = 0;
 ...
 */
function ccs_import_admin_page_content() {
    $imported = get_option('ccs_data_imported');
    $import_date = get_option('ccs_data_import_date');
    $courses_count = get_option('ccs_courses_imported_count', 0);
    $events_count = get_option('ccs_events_imported_count', 0);
    $news_count = get_option('ccs_news_imported_count', 0);
    $pages_count = get_option('ccs_pages_created_count', 0);
    ...
    if (isset($_POST['ccs_create_missing_pages']) && check_admin_referer('ccs_create_missing_pages_nonce')) {
        $create_pages_result = ccs_create_static_pages();
    }

    // Fix single course/event URLs (rename shadow pages)
    if (isset($_POST['ccs_fix_single_urls']) && check_admin_referer('ccs_fix_single_urls_nonce')) {
        $fix_urls_result = ccs_fix_cpt_page_slug_conflict_manual();
    }

    // Clean up course categories (enforce 2 max)
    if (isset($_POST['ccs_cleanup_categories']) && check_admin_referer('ccs_cleanup_categories_nonce')) {
        $cleanup_categories_result = function_exists('ccs_cleanup_course_categories') ? ccs_cleanup_course_categories() : 0;
    }
    
    // Handle manual import
    if (isset($_POST['ccs_reimport']) && check_admin_referer('ccs_reimport_nonce')) {
        delete_option('ccs_data_imported');
        ccs_import_data_on_activation();
    ...
    if (isset($_POST['ccs_import_events_only']) && check_admin_referer('ccs_import_events_nonce')) {
        $events_imported = ccs_import_scheduled_events();
        $last_result = get_option('ccs_events_import_last_result');
    ...
    if (isset($_POST['ccs_bulk_match']) && check_admin_referer('ccs_bulk_match_nonce')) {
        $matched = ccs_bulk_match_images();
    ...
    if (isset($_POST['ccs_populate_courses']) && check_admin_referer('ccs_populate_courses', 'ccs_populate_courses_nonce')) {
        ...
        if (empty($courses_to_populate)) {
            ...
            $populate_result = ccs_populate_courses_from_json($courses_to_populate);
        }
    }
    
    // Handle article auto-population
    $articles_result = null;
    if (isset($_POST['ccs_create_articles']) && check_admin_referer('ccs_create_articles')) {
        if (function_exists('ccs_create_articles_from_resources')) {
            $articles_result = ccs_create_articles_from_resources();
        }
    }
    
    // Handle bulk session title updates
    $session_titles_result = null;
    if (isset($_POST['ccs_update_session_titles']) && check_admin_referer('ccs_update_session_titles')) {
        $session_titles_result = ccs_bulk_update_session_titles();
    }
    ...
    <div class="wrap cta-import-page">
                ...
                <div class="cta-import-fix-grid">
                    <div class="cta-import-fix-card">
                        ...
                        <form method="post" style="margin: 0;">
                            <?php wp_nonce_field('ccs_create_missing_pages_nonce'); ?>
                            <button type="submit" name="ccs_create_missing_pages" class="button button-secondary">Create missing pages</button>
                        </form>
                    </div>
                    <div class="cta-import-fix-card">
                        ...
                        <form method="post" style="margin: 0;">
                            <?php wp_nonce_field('ccs_fix_single_urls_nonce'); ?>
                            <button type="submit" name="ccs_fix_single_urls" class="button button-secondary">Fix single URLs</button>
                        </form>
                    </div>
                    <div class="cta-import-fix-card">
                        ...
                        <form method="post" style="margin: 0;">
                            <?php wp_nonce_field('ccs_cleanup_categories_nonce'); ?>
                            <button type="submit" name="ccs_cleanup_categories" class="button button-secondary">Clean up categories</button>
                        </form>
                    </div>
                </div>
            </div>
        ...
        <div class="postbox" id="cta-reference">
            ...
            <div class="inside cta-import-reference">
                <details>
                    ...
                    <ul>
                        <li><code>/wp-content/themes/ccs-theme/data/courses-database.json</code> — courses</li>
                        <li><code>/wp-content/themes/ccs-theme/data/scheduled-courses.json</code> — scheduled sessions</li>
                        <li><code>/wp-content/themes/ccs-theme/data/news-articles.json</code> — blog/news</li>
                    </ul>
                </details>
                <details>
        ...
        <form method="post">
            <?php wp_nonce_field('ccs_reimport_nonce'); ?>
            <p>
                    <button type="submit" name="ccs_reimport" class="button button-primary button-hero" onclick="return confirm('This will import any new courses from the JSON files. Existing courses will not be affected. Continue?');">
        ...
        <form method="post" style="margin-top: 15px;">
            <?php wp_nonce_field('ccs_import_events_nonce'); ?>
            <p>
                <button type="submit" name="ccs_import_events_only" class="button button-secondary">
        ...
        <form method="post" id="cta-populate-courses-form">
            <?php wp_nonce_field('ccs_populate_courses', 'ccs_populate_courses_nonce'); ?>
            ...
            <p style="margin-top: 20px;">
                <button type="submit" name="ccs_populate_courses" class="button button-primary" onclick="return confirm('This will populate all ACF fields for the selected course(s). Continue?');">
        ...
        <div class="postbox" id="cta-articles">
            ...
            <div class="inside">
                ...
                <?php
                if (function_exists('ccs_get_government_resources_for_articles')) {
                    $resources = ccs_get_government_resources_for_articles();
                    $existing_count = 0;
                ...
                <form method="post" style="margin-top: 20px;">
                    <?php wp_nonce_field('ccs_create_articles'); ?>
                    <p>
                        <button type="submit" name="ccs_create_articles" class="button button-primary" onclick="return confirm('This will create new blog articles from government resources. Articles that already exist will be skipped. Continue?');">
        ...
        <div class="postbox" id="cta-session-titles">
            ...
            <div class="inside">
                ...
                <form method="post" style="margin-top: 20px;">
                    <?php wp_nonce_field('ccs_update_session_titles'); ?>
                    <p>
                        <button type="submit" name="ccs_update_session_titles" class="button button-primary" onclick="return confirm('This will update all course event titles to match their linked course titles. Continue?');">
                            <span class="dashicons dashicons-update" style="vertical-align: middle; margin-right: 5px;"></span>
        ...
        if (function_exists('ccs_render_phase1_posts_section')) {
            ccs_render_phase1_posts_section();
        }
        ?>
        ...
        $match_result = get_transient('ccs_bulk_match_result');
        if ($match_result !== false) {
            echo '<div class="notice notice-success"><p>Matched ' . intval($match_result) . ' images to courses.</p></div>';
            delete_transient('ccs_bulk_match_result');
        }
        
        $missing_images = ccs_get_posts_missing_images();
        ?>
        ...
        <form method="post">
            <?php wp_nonce_field('ccs_bulk_match_nonce'); ?>
            <p>
                <input type="submit" name="ccs_bulk_match" class="button button-secondary" value="Match Uploaded Images to Courses">
            </p>
        </form>
 ...
 */
function ccs_reset_import() {
    if (current_user_can('manage_options') && isset($_GET['ccs_reset_import']) && wp_verify_nonce($_GET['_wpnonce'], 'ccs_reset_import')) {
        delete_option('ccs_data_imported');
        delete_option('ccs_data_import_date');
        delete_option('ccs_courses_imported_count');
        delete_option('ccs_events_imported_count');
        
        wp_redirect(admin_url('tools.php?page=cta-import-data&reset=1'));
        exit;
    }
}
add_action('admin_init', 'ccs_reset_import');
 ...
 */
function ccs_auto_match_uploaded_image($attachment_id) {
 ...
 */
function ccs_handle_bulk_match() {
    if (isset($_POST['ccs_bulk_match']) && check_admin_referer('ccs_bulk_match_nonce')) {
        $matched = ccs_bulk_match_images();
        set_transient('ccs_bulk_match_result', $matched, 60);
        wp_redirect(admin_url('tools.php?page=cta-import-data&matched=1'));
        exit;
    }
}
add_action('admin_init', 'ccs_handle_bulk_match');

/**
 * Get courses/posts missing featured images
 */
function ccs_get_posts_missing_images() {
    $posts = get_posts([
        'post_type' => ['course', 'post', 'team_member'],
        'posts_per_page' => -1,
        'meta_key' => '_original_image_path',
 ...
 */
function ccs_import_site_settings() {
    $json_file = CCS_THEME_DIR . '/data/site-settings.json';
    ...
    update_option('ccs_site_settings', $settings);
 ...
 */
function ccs_import_team_members() {
    $json_file = CCS_THEME_DIR . '/data/team-members.json';
 ...
 */
function ccs_configure_wordpress_settings() {
    // Permalink structure
    $permalink_structure = get_option('permalink_structure');
    if (empty($permalink_structure)) {
        update_option('permalink_structure', '/%postname%/');
 ...
 * Get site settings
 */
function ccs_get_site_settings($key = null) {
    $settings = get_option('ccs_site_settings', []);
 ...
 * Get team members
 */
function ccs_get_team_members($type = null) {
    $args = [
        'post_type' => 'team_member',
        'posts_per_page' => -1,
        'orderby' => 'menu_order',
 ...
 */
function ccs_parse_and_store_reviews() {
    ...
    if (!empty($reviews)) {
        update_option('ccs_all_reviews', $reviews);
 ...
 */
function ccs_populate_course_expanded_fields() {
...
add_action('after_switch_theme', 'ccs_populate_course_expanded_fields', 30);
 ...
 */
function ccs_populate_safeguarding_course_content() {
    if (!function_exists('get_field') || !function_exists('update_field')) {
        return false;
    }
...
add_action('after_switch_theme', 'ccs_populate_safeguarding_course_content', 35);
