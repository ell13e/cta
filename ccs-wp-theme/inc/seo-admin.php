...
function ccs_add_seo_admin_menu() {
    ...
    add_submenu_page(
        ...
        'ccs_seo_admin_verification_page'
    );
    ...
    if (function_exists('ccs_seo_tools_admin_page')) {
        add_submenu_page(
            'cta-seo',
            'SEO Tools',
            'Tools',
            'manage_options',
            'cta-seo-tools',
            'ccs_seo_tools_admin_page'
        );
    }
    
    // Sitemap Diagnostic (moved from Tools menu)
    // Note: Function is defined in seo.php, so we reference it directly
    if (function_exists('ccs_sitemap_diagnostic_page')) {
        add_submenu_page(
            'cta-seo',
            'Sitemap Diagnostic',
            'Sitemap Diagnostic',
    ...
    if (function_exists('ccs_performance_admin_page')) {
        add_submenu_page(
            'cta-seo',
 ...
 */
function ccs_seo_admin_enqueue_styles($hook) {
    ...
    wp_enqueue_style(
        ...
        [],
        defined('CCS_THEME_VERSION') ? CCS_THEME_VERSION : '1.0.0'
    );
}
add_action('admin_enqueue_scripts', 'ccs_seo_admin_enqueue_styles', 11);
...
function ccs_seo_settings_page() {
    ...
    if (isset($_POST['ccs_save_seo_settings']) && check_admin_referer('ccs_seo_settings')) {
        ...
        update_option('ccs_seo_title_separator', $title_separator);
        update_option('ccs_seo_default_robots_index', $default_robots_index);
        update_option('ccs_seo_default_robots_follow', $default_robots_follow);
        
        echo '<div class="notice notice-success"><p>Settings saved!</p></div>';
    }
    
    $title_separator = get_option('ccs_seo_title_separator', '–');
    $default_robots_index = get_option('ccs_seo_default_robots_index', 1);
    $default_robots_follow = get_option('ccs_seo_default_robots_follow', 1);
    ...
    <div class="wrap cta-seo-page">
        ...
        <div class="cta-seo-section">
            ...
            <form method="post">
                <?php wp_nonce_field('ccs_seo_settings'); ?>
                ...
                <?php submit_button('Save settings', 'primary', 'ccs_save_seo_settings'); ?>
            </form>
            </div>
        </div>
 ...
 */
function ccs_seo_bulk_meta_field($post_type) {
    $fields = [
 ...
 */
function ccs_seo_bulk_page() {
    // Single-item quick action (GET)
    if (isset($_GET['ccs_bulk_single']) && isset($_GET['id']) && isset($_GET['action']) && check_admin_referer('ccs_bulk_seo_single_' . (int) $_GET['id'])) {
        $post_id = (int) $_GET['id'];
        ...
        if ($post && current_user_can('edit_post', $post_id)) {
            if ($action === 'generate_descriptions') {
                ...
                $field = ccs_seo_bulk_meta_field($post->post_type);
                if (!empty($description)) {
                    ccs_safe_update_field($field, $description, $post_id);
                }
            } elseif ($action === 'apply_schema') {
                $schema_type = ccs_get_schema_template($post->post_type);
                if ($schema_type && $schema_type !== 'None') {
                    ccs_safe_update_field('page_schema_type', $schema_type, $post_id);
                }
            }
    ...
    if (isset($_POST['ccs_bulk_seo_action']) && check_admin_referer('ccs_bulk_seo')) {
        ...
        if (empty($post_ids)) {
            ...
            foreach ($post_ids as $post_id) {
                ...
                $meta_field = ccs_seo_bulk_meta_field($post->post_type);

                switch ($action) {
                    case 'generate_descriptions':
                        $description = ccs_get_meta_description($post);
                        if (!empty($description)) {
                            ccs_safe_update_field($meta_field, $description, $post_id);
                            $updated++;
                        } else {
                            $skipped++;
                        }
    ...
    if ($filter_status === 'missing_meta') {
        $meta_key = ccs_seo_bulk_meta_field($filter_post_type);
        $args['meta_query'] = [
            'relation' => 'OR',
            [
                'key'     => $meta_key,
    ...
    <div class="wrap cta-seo-page">
        ...
        <div class="cta-seo-toolbar">
            ...
            <form method="post" id="cta-bulk-seo-form" class="cta-seo-toolbar-actions">
                <?php wp_nonce_field('ccs_bulk_seo'); ?>
                <input type="hidden" name="ccs_bulk_seo_action" value="1" />
                    ...
                    <table class="wp-list-table widefat fixed striped">
                        ...
                        <tbody>
                            ...
                            while ($query->have_posts()) :
                                ...
                                $meta_field = ccs_seo_bulk_meta_field($pt);
                                $meta_title = $pt === 'course' ? ccs_safe_get_field('course_seo_meta_title', $post_id, '') : ($pt === 'course_event' ? ccs_safe_get_field('event_seo_meta_title', $post_id, '') : ccs_safe_get_field('page_seo_meta_title', $post_id, ''));
                                $meta_description = ccs_safe_get_field($meta_field, $post_id, '');
                                $schema_type = ccs_safe_get_field('page_schema_type', $post_id, '');
                                $single_nonce = wp_create_nonce('ccs_bulk_seo_single_' . $post_id);
                                $base_url = admin_url('admin.php');
                                $gen_url = add_query_arg(['page' => 'cta-seo-bulk', 'ccs_bulk_single' => 1, 'id' => $post_id, 'action' => 'generate_descriptions', '_wpnonce' => $single_nonce], $base_url);
                                $schema_url = add_query_arg(['page' => 'cta-seo-bulk', 'ccs_bulk_single' => 1, 'id' => $post_id, 'action' => 'apply_schema', '_wpnonce' => $single_nonce], $base_url);
                            ?>
                                <tr>
                                    <td>
 ...
 */
function ccs_seo_sitemap_page() {
    // Use existing sitemap function
    if (function_exists('ccs_sitemap_admin_page')) {
        ccs_sitemap_admin_page();
    } else {
 ...
 */
function ccs_seo_schema_page() {
    global $wpdb;

    $pages_with_custom_schema = (int) $wpdb->get_var("
        SELECT COUNT(*) FROM {$wpdb->postmeta}
        WHERE meta_key = '_ccs_schema_type' AND meta_value != ''
    ");
 ...
 * SEO Images Page
 */
function ccs_seo_images_page() {
    global $wpdb;
    
    // Handle bulk alt text action
    if (isset($_POST['ccs_bulk_add_alt_text']) && check_admin_referer('ccs_bulk_alt_text_action')) {
    ...
    if (isset($_POST['ccs_bulk_remove_alt_text']) && check_admin_referer('ccs_bulk_remove_alt_text_action')) {
        $images = $wpdb->get_results("
    ...
    <div class="wrap cta-seo-page">
        ...
        <div class="cta-seo-section">
            ...
            <form method="post">
                <?php wp_nonce_field('ccs_bulk_alt_text_action'); ?>
                <p>
                    <button type="submit" name="ccs_bulk_add_alt_text" class="button button-primary">
        ...
        <div class="cta-seo-section">
            ...
            <form method="post" onsubmit="return confirm('Remove alt text from all <?php echo number_format($images_with_alt); ?> images? This cannot be undone.');">
                <?php wp_nonce_field('ccs_bulk_remove_alt_text_action'); ?>
                <p>
                    <button type="submit" name="ccs_bulk_remove_alt_text" class="button cta-seo-btn-danger">
 ...
 */
function ccs_seo_admin_verification_page() {
    // The function ccs_seo_verification_page() is defined in seo-verification.php
    // Since seo-verification.php is loaded before this file, we can call it directly
    if (function_exists('ccs_seo_verification_page')) {
        // Call the original function from seo-verification.php
        call_user_func('ccs_seo_verification_page');
    } else {
...
function ccs_seo_redirects_page() {
    if (isset($_POST['ccs_save_redirects']) && check_admin_referer('ccs_seo_redirects')) {
        $redirect_attachments = isset($_POST['redirect_attachments']) ? 1 : 0;
        $strip_category_base = isset($_POST['strip_category_base']) ? 1 : 0;
        update_option('ccs_redirect_attachments_enabled', $redirect_attachments);
        update_option('ccs_strip_category_base_enabled', $strip_category_base);
        flush_rewrite_rules();
        echo '<div class="notice notice-success is-dismissible"><p>Redirect settings saved.</p></div>';
    }

    $redirect_attachments = (int) get_option('ccs_redirect_attachments_enabled', 1);
    $strip_category_base = (int) get_option('ccs_strip_category_base_enabled', 1);
    ?>
    <div class="wrap cta-seo-page">
        <header class="cta-seo-header">
            <h1>Redirects</h1>
        ...
        <div class="cta-seo-section">
            ...
            <div class="cta-seo-section__body">
                <form method="post">
                    <?php wp_nonce_field('ccs_seo_redirects'); ?>
                    <table class="form-table">
                        <tr>
                    ...
                    <?php submit_button('Save settings', 'primary', 'ccs_save_redirects'); ?>
                </form>
            </div>
        </div>
