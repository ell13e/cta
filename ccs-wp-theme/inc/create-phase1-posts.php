 ...
 */
function ccs_get_or_create_category($name, $slug = '') {
    if (empty($slug)) {
 ...
 */
function ccs_convert_markdown_to_html($content) {
 ...
 */
function ccs_extract_article_from_markdown($markdown_file, $article_number) {
 ...
 */
function ccs_create_phase1_blog_posts() {
    ...
    foreach ($categories as $slug => $name) {
        $cat_id = ccs_get_or_create_category($name, $slug);
        if ($cat_id) {
    ...
    foreach ($articles as $article) {
        ...
        if ($article['status'] === 'publish') {
            $content = ccs_extract_article_from_markdown($markdown_file, $article['article_number']);
        }
        ...
        $html_content = ccs_convert_markdown_to_html($content);
        ...
        if (function_exists('update_post_meta')) {
            ...
            update_post_meta($post_id, '_ccs_article_keywords', $article['keywords']);
        }
